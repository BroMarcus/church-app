...
export default async function ProphetPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const l=(p:string)=>{if(!es)return p;const [base,hash]=p.split('#');return `${base}${base.includes('?')?'&':'?'}lang=es${hash?`#${hash}`:''}`}
  ...
  const [{data:assignments},{count:unread},{data:ledGroups},{data:groupRoles},{data:activeLearning}]=await Promise.all([
    ...
    supabase.from('course_enrollments').select('course_id,updated_at').eq('user_id',userId).eq('credential_earned',false).order('updated_at',{ascending:false}).limit(1)
  ])
  ...
  const learningHref=learningResume?.href??'/learning'
  const learningLabel=learningResume?.kind==='lesson'
    ?`${es?'Continuar':'Continue'}: ${learningResume.moduleTitle??(es?'Siguiente lección':'Next lesson')}`
    :learningResume?.kind==='final'
      ?(es?'Examen final listo':'Final exam ready')
      :learningResume?.kind==='complete'
        ?(es?'Curso listo para terminar':'Course ready to finish')
        :(es?'Continuar una clase o ver el siguiente paso.':'Continue a course or see the next step.')
  const links:any[]=[
    ...
    [es?'Aprendizaje':'Learning',learningLabel,learningHref,BookOpen],
  ...
  return <main className="shell">
    ...
    <ProphetCommandBox lang={lang} learningHref={learningResume?.href??null} learningLabel={learningResume?learningLabel:null}/>
    <section className="card" style={{padding:18}}><div className="pill">{es?'ACCIONES RÁPIDAS':'QUICK ACTIONS'}</div><h2>{es?'O toca una acción y sigue adelante.':'Or tap an action and keep moving.'}</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:10,marginTop:12}}>{links.map(([title,body,href,Icon])=><Link key={`${title}-${href}`} href={l(href)} className="card" style={{padding:14,textDecoration:'none'}}><Icon size={18}/><strong style={{display:'block',marginTop:8}}>{title}</strong><span className="small muted">{body}</span></Link>)}</div></section>
  </main>
}
