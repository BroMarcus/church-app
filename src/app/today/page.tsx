...
export default async function MyTodayPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const [{data:milestones},{count:groupCount},{data:assignments},{data:enrollments},{data:newConvertCourses},{count:unread},{data:ledGroups},{data:groupRoles},{data:overdueOutreach},{data:assignedOutreach}]=await Promise.all([
  ...
  const sessionRows=sessions??[],communicationRows=communicationIssues??[]
  ...
  const urgentCount=pending.length+overdueGroups.length+outreachRows.length+communicationRows.length
