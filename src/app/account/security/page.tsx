...
export default async function AccountSecurityPage({searchParams}:{searchParams:Promise<{email?:string;password?:string;status?:string;error?:string;lang?:string;next?:string}>}){
  const query=await searchParams,es=query.lang==='es',t=(en:string,sp:string)=>es?sp:en,joinNext=safeJoinNext(query.next),nextPart=joinNext?`&next=${encodeURIComponent(joinNext)}`:''
  const l=(path:string)=>es?`${path}${path.includes('?')?'&':'?'}lang=es`:path
  const securityHref=(lang:'en'|'es')=>`/account/security?lang=${lang}${nextPart}`
  ...
  if(!userId)redirect(`/login?lang=${es?'es':'en'}&mode=signin${nextPart}`)
  ...
  return <main className="shell">
    ...
    {joinNext&&<div className="notice" role="status"><strong>{t('Finish your church connection','Termina tu conexión con la iglesia')}</strong><div className="small" style={{marginTop:4}}>{t('After you sign out everywhere, we will return you to Sign in and then back to the church you were joining.','Después de cerrar sesión en todos los dispositivos, te regresaremos a Iniciar sesión y luego a la iglesia a la que estabas tratando de unirte.')}</div></div>}
