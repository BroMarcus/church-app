...
export async function verifyDocument(formData:FormData){
  ...
  const {error}=await supabase.from('member_documents').update({verification_status:status,verification_notes:text(formData,'verification_notes')||null,verified_by:status==='verified'?userId:null,verified_at:status==='verified'?now:null,updated_at:now}).eq('id',documentId)
