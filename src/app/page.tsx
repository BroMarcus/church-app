...
import { ArrowRight,BookOpen,BriefcaseBusiness,CalendarDays,Church,FileText,Globe2,GraduationCap,HandHeart,HeartHandshake,MessageCircle,Megaphone,ShieldCheck,Sparkles,Users } from 'lucide-react'
...
const modules=[
  {title:'Start Here',desc:'Simple first-time guide • English / Español',Icon:Sparkles,href:'/start'},
  {title:'My Journey',desc:'Your discipleship pathway',Icon:Sparkles,href:'/journey'},
  {title:'Learning',desc:'Courses, badges & growth',Icon:GraduationCap,href:'/learning'},
  {title:'Kingdom Guide',desc:'Navigation & trusted resources',Icon:BookOpen,href:'/guide'},
  {title:'Groups',desc:'Friendship groups & reports',Icon:Users,href:'/groups'},
  {title:'Prayer & Testimony',desc:'Share prayer and victories',Icon:HandHeart,href:'/prayer'},
  ...
  {title:'Private Care',desc:'Prayer & pastoral support',Icon:HandHeart,href:'/help'}
] as const
...
export default async function Home(){
  ...
  if(isAdmin){const [{count:care},{count:reports},{count:outreach},{count:docs},{count:applications}]=await Promise.all([
  ...
  const newConvertIds=(newConvertCourses??[]).map((c:any)=>c.id);let newConvertCompleted=false
  ...
  return <main className="shell">
    ...
    {isAdmin&&<section className="card" style={{padding:20,marginBottom:18,borderColor:accent,display:'flex',justifyContent:'space-between',alignItems:'center',gap:18,flexWrap:'wrap'}}><div style={{display:'flex',gap:13,alignItems:'flex-start',maxWidth:760}}><div className="avatar" style={{width:46,height:46,flex:'none'}}><ShieldCheck size={20}/></div><div><div className="pill">LEADERSHIP TODAY</div><h2 style={{margin:'9px 0 5px'}}>{leadershipNeeds>0?`${leadershipNeeds} item${leadershipNeeds===1?'':'s'} need attention`:'Leadership queue is clear'}</h2><p className="muted" style={{margin:0,lineHeight:1.5}}>{leadershipNeeds>0?'Pastoral care, outreach, reports, documents and ministry applications are waiting. Start with the highest-priority item.':'Nothing urgent is waiting in the main leadership queues right now.'}</p></div></div><Link className="btn" href="/church/leadership" style={{display:'inline-flex',alignItems:'center',gap:7}}>{leadershipNeeds>0?'Do this first':'Open leadership'}<ArrowRight size={15}/></Link></section>}
    ...
    <div className="content-grid"><CommunityFeed churchId={membership.church_id} userId={userId}/><aside><div className="card side"><div className="pill">ACTIVE NOW</div><h3>Kingdom Network Alpha</h3><ul><li>Bilingual Start Here onboarding for first-time members</li><li>Personalized My Next Step + My Journey</li><li>Kingdom Guide trusted-resource Beta</li><li>Prayer & Testimony Wall + private pastoral care</li><li>Personalized upcoming responsibilities</li><li>Featured event discovery & flyers</li><li>Organization & district Network layers</li><li>Private member messaging with block/report controls</li><li>Member privacy, security & notification preferences</li><li>Church-specific logo, branding & welcome</li><li>Official church updates & network updates</li><li>Member profiles, Directory & secure invitations</li><li>Church admin, verified records & audit history</li><li>Friendship Groups, join requests & leader reports</li><li>First Steps Learning Center</li><li>Learning Studio, levels, trophies & games</li><li>Weekly learning challenges & streaks</li><li>Outreach follow-up pipeline, history & member conversion</li><li>Ministry opportunities & qualification</li><li>Unified church/district/organization calendar & RSVPs</li><li>Team schedules & confirmations</li><li>Fundraising campaigns & goal tracking</li><li>Private document vault & verification</li><li>CSV import/export + member My Data</li><li>Community feed with comments & reactions</li></ul></div><div className="card side"><div className="pill">CURRENT BUILD FOCUS</div><h3>Pilot readiness & validation</h3><p className="muted">Finish cross-module navigation and privacy wiring, validate onboarding and auth recovery on real phones, stabilize deployment, then run a real New Life pilot review.</p></div></aside></div>
  </main>
}
