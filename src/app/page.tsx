...
export default async function Home(){
  ...
  return <main className="shell">
    ...
    <section className="module-grid">{modules.map(({title,desc,Icon,...module})=>{const body=<><Icon size={22}/><strong>{title}</strong><span>{desc}</span><small>Open module</small></>;return <Link className="module card module-link" href={module.href} key={title}>{body}</Link>})}</section>
