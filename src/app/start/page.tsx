...
export default async function StartPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const steps=[
    {title:t.profile,body:t.profileBody,href:'/profile',Icon:UserRound},
    {title:t.journey,body:t.journeyBody,href:'/journey',Icon:Sparkles},
    {title:t.groups,body:t.groupsBody,href:'/groups',Icon:Users},
    {title:t.calendar,body:t.calendarBody,href:'/calendar',Icon:CalendarDays},
    {title:t.guide,body:t.guideBody,href:`/guide${lang==='es'?'?lang=es':''}`,Icon:BookOpen},
    {title:t.care,body:t.careBody,href:'/help',Icon:HandHeart}
  ]

  return <main className="shell start-shell">
    <header className="topbar"><div><Link href="/" className="brand">Kingdom <span>Network</span></Link><div className="small muted">{church?.name??t.church} • Start Here</div></div><div className="row"><Languages size={15}/><Link className="ghost" href="/start?lang=en">{t.english}</Link><Link className="ghost" href="/start?lang=es">{t.spanish}</Link></div></header>
    ...
    <section><div className="section-heading"><div><div className="pill">NEXT STEP</div><h2>{t.choose}</h2></div></div><div className="start-grid">{steps.map(({title,body,href,Icon})=><Link className="card start-step" href={href} key={title}><div className="start-icon"><Icon size={19}/></div><div><strong>{title}</strong><span>{body}</span></div></Link>)}</div></section>

    <section className="card start-note"><p>{t.note}</p><Link className="btn" href="/">{t.home}</Link></section>
  </main>
}
