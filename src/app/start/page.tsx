...
const copy={
 en:{title:"You're in.",subtitle:'You can finish your profile and explore the app later. For now, start with Home.',church:'Your church',member:'Your account is connected',account:'Signed in as',accountHint:'Keep using this same One Kingdom account. Do not create another account for this church.',walk:'OPTIONAL: ADD MORE LATER',tour:'OPTIONAL: LEARN THE APP',tourBody:'You do not need to memorize the app. These links stay here whenever you want them.',profile:'Check your profile',profileBody:'Confirm your name or add more information whenever you are ready.',journey:'Open My Journey',journeyBody:'See your discipleship progress, milestones and next steps.',groups:'Find my Friendship Group',groupsBody:'See your group, meeting information, lesson, prayer wall and check-in.',home:'Home',homeBody:'Your simple starting point for what matters now.',learning:'Learning Center',learningBody:'Continue church classes and see your progress.',guide:'Kingdom Guide',guideBody:'If you get lost, ask a normal question and get pointed to the right place.',prayer:'Prayer & Testimony',prayerBody:'Share a prayer request or testimony with the privacy level you choose.',updates:'Official Updates',updatesBody:'Read important church announcements.',alerts:'Alerts',alertsBody:'See reminders that need your attention.',feedback:'Share an idea',feedbackBody:'Tell us what is confusing, broken, useful or missing during the pilot.',care:'Private Care',careBody:'Ask for sensitive pastoral help in a restricted area.',quickTitle:'Start with Home.',quickBody:'You do not have to fill anything else out right now. Home will show the next useful thing when you need it.',finish:'Go to Home',saving:'Saving — keep this page open…',spanish:'Español',english:'English',ready:'Account ready',error:'Something needs attention',start:'Start Here',adminPill:'SETTING UP THE CHURCH?',adminTitle:'Continue Church Builder',adminBody:'Church Builder gives authorized leaders one setup step at a time. Member setup can wait.',adminButton:'Open Church Builder',connectionTitle:'We could not safely load Start Here.',connectionBody:'Nothing was changed. This is usually temporary. Try again; if it continues, return to Sign in.',retry:'Try Start Here again',signIn:'Go to Sign in',helpTitle:'NEED HELP?',helpSummary:'Need help or feel lost?',optionalHint:'Only open these if you want to do more right now.'},
 es:{title:'Ya entraste.',subtitle:'Puedes completar tu perfil y explorar la aplicación después. Por ahora, empieza con Inicio.',church:'Tu iglesia',member:'Tu cuenta está conectada',account:'Sesión iniciada como',accountHint:'Sigue usando esta misma cuenta de One Kingdom. No crees otra cuenta para esta iglesia.',walk:'OPCIONAL: AGREGA MÁS DESPUÉS',tour:'OPCIONAL: CONOCE LA APLICACIÓN',tourBody:'No necesitas memorizar la aplicación. Estos enlaces estarán aquí cuando los quieras.',profile:'Revisa tu perfil',profileBody:'Confirma tu nombre o agrega más información cuando estés listo.',journey:'Abre Mi Camino',journeyBody:'Mira tu progreso de discipulado, hitos y próximos pasos.',groups:'Encuentra mi Grupo de Amistad',groupsBody:'Mira tu grupo, reunión, lección, muro de oración y registro de asistencia.',home:'Inicio',homeBody:'Tu punto de partida sencillo para lo que importa ahora.',learning:'Centro de Aprendizaje',learningBody:'Continúa tus clases de iglesia y mira tu progreso.',guide:'Kingdom Guide',guideBody:'Si te pierdes, haz una pregunta normal y te llevará al lugar correcto.',prayer:'Oración y Testimonio',prayerBody:'Comparte una petición o testimonio con el nivel de privacidad que tú elijas.',updates:'Actualizaciones Oficiales',updatesBody:'Lee anuncios importantes de la iglesia.',alerts:'Alertas',alertsBody:'Mira recordatorios que necesitan tu atención.',feedback:'Comparte una idea',feedbackBody:'Dinos qué es confuso, qué no funciona, qué ayuda o qué falta durante el piloto.',care:'Cuidado Privado',careBody:'Pide ayuda pastoral sensible en un área restringida.',quickTitle:'Empieza con Inicio.',quickBody:'No tienes que llenar nada más ahora. Inicio te mostrará lo siguiente que sea útil cuando lo necesites.',finish:'Ir a Inicio',saving:'Guardando — mantén esta página abierta…',spanish:'Español',english:'English',ready:'Cuenta lista',error:'Algo necesita atención',start:'Empieza Aquí',adminPill:'¿ESTÁS CONFIGURANDO LA IGLESIA?',adminTitle:'Continuar Church Builder',adminBody:'Church Builder da a los líderes autorizados un paso de configuración a la vez. La configuración del miembro puede esperar.',adminButton:'Abrir Church Builder',connectionTitle:'No pudimos cargar Empieza Aquí de forma segura.',connectionBody:'No se cambió nada. Normalmente esto es temporal. Inténtalo otra vez; si continúa, vuelve a Iniciar sesión.',retry:'Intentar Empieza Aquí otra vez',signIn:'Ir a Iniciar sesión',helpTitle:'¿NECESITAS AYUDA?',helpSummary:'¿Necesitas ayuda o estás perdido?',optionalHint:'Abre estas opciones solamente si quieres hacer más ahora.'}
} as const
...
const messageCopy={
 en:{joined_existing:'You are connected to this church with your existing One Kingdom account. Keep using this same account—do not create another one.',already_joined:'You are already connected to this church. Keep using this same One Kingdom account—no second account is needed.',joined_invite:'Your email is confirmed and this church invitation is connected to your One Kingdom account. Keep using this same account—no second account is needed.'},
 es:{joined_existing:'Ya estás conectado a esta iglesia con tu cuenta existente de One Kingdom. Sigue usando esta misma cuenta—no crees otra.',already_joined:'Ya estabas conectado a esta iglesia. Sigue usando esta misma cuenta de One Kingdom—no necesitas una segunda cuenta.',joined_invite:'Tu correo está confirmado y esta invitación de la iglesia está conectada a tu cuenta de One Kingdom. Sigue usando esta misma cuenta—no necesitas una segunda cuenta.'}
} as const
...
export default async function StartPage({searchParams}:{searchParams:Promise<{lang?:string;error_code?:string;message_code?:string}>}){
 ...
 <section className="card start-hero"><div><div className="pill">{t.start.toUpperCase()}</div><h1>{t.title}</h1><p className="muted">{t.subtitle}</p><p className="small muted" style={{marginTop:10}}>{name} • {church?.name??t.church} • {roleLabel(membership.role,lang)}</p>{user.email&&<p className="small" style={{marginTop:8}}><strong>{t.account}:</strong> {user.email}</p>}<p className="small muted" style={{marginTop:6}}>{t.accountHint}</p></div><div className="start-ready"><CheckCircle2 size={28}/><strong>{t.ready}</strong></div></section>{statusError&&<div className="notice error" role="alert"><strong>{t.error}:</strong> {statusError}</div>}{statusMessage&&<div className="notice success" role="status" aria-live="polite">{statusMessage}</div>}
 ...
 <details className="card start-how"><summary style={{cursor:'pointer',fontWeight:800}}>{t.helpSummary}</summary><div className="pill start-section-label" style={{marginTop:14}}>{t.helpTitle}</div><div className="start-grid"><Link className="card start-step" href={withLang('/guide')}><div className="start-icon"><Sparkles size={19}/></div><div><strong>{t.guide}</strong><span>{t.guideBody}</span></div></Link><Link className="card start-step" href={withLang('/help')}><div className="start-icon"><HandHeart size={19}/></div><div><strong>{t.care}</strong><span>{t.careBody}</span></div></Link><Link className="card start-step" href={withLang('/feedback')}><div className="start-icon"><MessageSquareWarning size={19}/></div><div><strong>{t.feedback}</strong><span>{t.feedbackBody}</span></div></Link></div></details>
 </main>
}