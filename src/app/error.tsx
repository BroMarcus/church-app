  ...
  return <main style={{maxWidth:760,margin:'0 auto',padding:'28px 18px 80px'}}>
    <div role="alert" style={{border:'1px solid #fecaca',borderRadius:18,padding:24,background:'#fff'}}>
      ...
      <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
        ...
        <button type="button" onClick={()=>go('/')} style={{minHeight:44,padding:'10px 16px',borderRadius:10,border:'1px solid #d1d5db',background:'#fff',fontWeight:700,cursor:'pointer'}}>Home / Inicio</button>
        <button type="button" onClick={()=>go('/login?mode=signin')} style={{minHeight:44,padding:'10px 16px',borderRadius:10,border:'1px solid #d1d5db',background:'#fff',fontWeight:700,cursor:'pointer'}}>Sign in / Iniciar sesión</button>
      </div>
    </div>
  </main>
}
