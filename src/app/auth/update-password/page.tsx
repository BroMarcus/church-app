  ...
  useEffect(()=>{
    ...
    const check=async()=>{
      try{
        ...
        if(!mounted)return
        if(data.session){setAccountEmail(data.session.user.email||'');setRetryAvailable(false);setReady(true);setMessage(c.choose)}else{setRetryAvailable(false);setReady(false);setMessage(c.invalidBack)}
      }catch(error){
    ...
    void check()
    const {data:listener}=supabase.auth.onAuthStateChange((event,session)=>{if(!mounted)return;if((event==='PASSWORD_RECOVERY'||event==='SIGNED_IN')&&session){const nextUrl=new URL(window.location.href),listenerLang=preferredLanguage(nextUrl.searchParams);setLang(listenerLang);setJoinNext(safeJoinNext(nextUrl.searchParams.get('next')));setInviteId(safeInviteId(nextUrl.searchParams.get('invite')));setAccountEmail(session.user.email||'');setRetryAvailable(false);setReady(true);setMessage(copy[listenerLang].choose)}})
