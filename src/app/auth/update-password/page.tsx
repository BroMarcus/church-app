...
const copy={
  en:{opening:'Opening your secure reset link…',invalid:'This reset link is invalid or expired. Please request one fresh reset email.',choose:'Choose a new password below.',invalidBack:'This reset link is invalid or expired. Go back to Sign in and request one fresh reset email.',sessionUnavailable:'We could not safely check your reset session right now. Keep this page open and try again once. If it still does not work, return to Sign in and request one fresh reset email.',accountMissing:'We opened a reset session but could not safely confirm which email account it belongs to. Do not change a password here. Go back to Sign in and request one fresh reset email for the correct account.',accountChanged:'The signed-in account changed while this reset page was open. Nothing was changed. Go back to Sign in and request a fresh reset for the account you want to change.',short:'Password must be at least 8 characters.',tooLong:'Password must be 128 characters or fewer.',mismatch:'The two passwords do not match.',failed:'We could not update the password right now. Keep this page open and try once more. If the reset link later expires, request one fresh reset email.',updated:'Password updated. Your old password will no longer work.',success:'Password updated. Continue to sign in with your new password.',signOutIncomplete:'Your password was updated, but we could not safely finish signing this browser out. Open Account Security, choose “Sign out everywhere,” then sign in again with your new password.',title:'Reset your password',account:'Resetting password for',accountHint:'Make sure this is the One Kingdom account you meant to reset. If the email is wrong, do not continue—go back to Sign in and request a reset for the correct account.',newPassword:'New password',again:'Type it again',showPassword:'Show password',hidePassword:'Hide password',updating:'Updating…',update:'Update password',retry:'Try again',continue:'Continue to sign in',accountSecurity:'Open Account Security',back:'Back to sign in'},
  es:{opening:'Abriendo tu enlace seguro…',invalid:'Este enlace no es válido o ya venció. Solicita un correo nuevo para cambiar tu contraseña.',choose:'Escribe una contraseña nueva abajo.',invalidBack:'Este enlace no es válido o ya venció. Vuelve a Iniciar sesión y solicita un correo nuevo.',sessionUnavailable:'No pudimos verificar de forma segura tu sesión para cambiar la contraseña. Mantén esta página abierta e inténtalo una vez más. Si todavía no funciona, vuelve a Iniciar sesión y solicita un correo nuevo.',accountMissing:'Abrimos una sesión para cambiar la contraseña, pero no pudimos confirmar de forma segura a qué correo pertenece. No cambies ninguna contraseña aquí. Vuelve a Iniciar sesión y solicita un correo nuevo para la cuenta correcta.',accountChanged:'La cuenta con sesión iniciada cambió mientras esta página estaba abierta. No se cambió nada. Vuelve a Iniciar sesión y solicita un cambio nuevo para la cuenta que quieres usar.',short:'La contraseña debe tener al menos 8 caracteres.',tooLong:'La contraseña debe tener 128 caracteres o menos.',mismatch:'Las dos contraseñas no coinciden.',failed:'No pudimos cambiar la contraseña en este momento. Mantén esta página abierta e inténtalo una vez más. Si después vence el enlace, solicita un correo nuevo.',updated:'Contraseña actualizada. Tu contraseña anterior ya no funcionará.',success:'Contraseña actualizada. Continúa para iniciar sesión con tu nueva contraseña.',signOutIncomplete:'Tu contraseña fue actualizada, pero no pudimos cerrar esta sesión del navegador de forma segura. Abre Seguridad de la Cuenta, elige “Cerrar sesión en todas partes” y luego inicia sesión otra vez con tu contraseña nueva.',title:'Cambiar tu contraseña',account:'Cambiando la contraseña para',accountHint:'Confirma que esta sea la cuenta de One Kingdom que querías cambiar. Si el correo está equivocado, no continúes—vuelve a Iniciar sesión y solicita el cambio para la cuenta correcta.',newPassword:'Nueva contraseña',again:'Escríbela otra vez',showPassword:'Mostrar contraseña',hidePassword:'Ocultar contraseña',updating:'Actualizando…',update:'Actualizar contraseña',retry:'Intentar otra vez',continue:'Continuar a Iniciar sesión',accountSecurity:'Abrir Seguridad de la Cuenta',back:'Volver a Iniciar sesión'}
} as const
...
export default function UpdatePasswordPage(){
  ...
  useEffect(()=>{
    ...
    const acceptSession=(session:{user:{email?:string|null}})=>{
      ...
      if(!email){setAccountEmail('');setRetryAvailable(false);setReady(false);setMessage(c.accountMissing);return false}
      setAccountEmail(email);setRetryAvailable(false);setReady(true);setMessage(c.choose);return true
    }
    const check=async()=>{
      try{
    ...
    void check()
    const {data:listener}=supabase.auth.onAuthStateChange((event,session)=>{if(!mounted)return;if((event==='PASSWORD_RECOVERY'||event==='SIGNED_IN')&&session){const nextUrl=new URL(window.location.href),listenerLang=preferredLanguage(nextUrl.searchParams);setLang(listenerLang);setJoinNext(safeJoinNext(nextUrl.searchParams.get('next')));setInviteId(safeInviteId(nextUrl.searchParams.get('invite')));const email=(session.user.email||'').trim();if(!email){setAccountEmail('');setRetryAvailable(false);setReady(false);setMessage(copy[listenerLang].accountMissing);return}setAccountEmail(email);setRetryAvailable(false);setReady(true);setMessage(copy[listenerLang].choose)}})
    return()=>{mounted=false;listener.subscription.unsubscribe()}
  },[])

  async function save(e:React.FormEvent){
    e.preventDefault();if(!accountEmail){setReady(false);setMessage(t.accountMissing);return}if(password.length<8){setMessage(t.short);return}if(password.length>128||confirm.length>128){setMessage(t.tooLong);return}if(password!==confirm){setMessage(t.mismatch);return}
    setBusy(true)
    ...
    try{
      ...
      const currentEmail=(identity.data.user?.email||'').trim()
      if(identity.error||!identity.data.user?.id||!currentEmail||currentEmail.toLowerCase()!==accountEmail.toLowerCase()){
        console.error('password reset account identity changed',{code:identity.error?diagnosticCode(identity.error):'identity_mismatch'})
        setReady(false);setMessage(t.accountChanged);return
      }
      const {data,error}=await supabase.auth.updateUser({password})
      if(error){console.error('password update failed',{code:diagnosticCode(error)});setMessage(t.failed);return}
      if(!data?.user||data.user.id!==identity.data.user.id){console.error('password update returned incomplete or changed auth state',{code:'auth_state_mismatch'});setMessage(t.failed);return}
