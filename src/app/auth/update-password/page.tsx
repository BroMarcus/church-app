...
export default function UpdatePasswordPage(){
  ...
  const nextPart=joinNext?`&next=${encodeURIComponent(joinNext)}`:''
  return <main className="login-wrap"><div className="login card"><div className="pill">ONE KINGDOM</div><h1>{t.title}</h1><div className={`notice ${ready?'success':'error'}`} role="status" aria-live="polite">{message}</div>{ready&&<form onSubmit={save}><PasswordField name="password" label={t.newPassword} minLength={8} autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)} required showLabel={t.showPassword} hideLabel={t.hidePassword}/><PasswordField name="confirm_password" label={t.again} minLength={8} autoComplete="new-password" value={confirm} onChange={e=>setConfirm(e.target.value)} required showLabel={t.showPassword} hideLabel={t.hidePassword}/><button className="btn" type="submit" disabled={busy}>{busy?t.updating:t.update}</button></form>}<p className="small muted" style={{marginTop:16}}><a href={`/login?lang=${lang}&mode=signin${nextPart}`}>{t.back}</a></p></div></main>
}
