...
export default async function InvitesPage({searchParams}:{searchParams:Promise<{created?:string;revoked?:string;error?:string;lang?:string}>}){
  ...
  const {data:signupData}=church?.slug?await supabase.rpc('get_public_signup_status_for_church',{p_church_slug:church.slug}):{data:null}
  const signup:any=Array.isArray(signupData)?signupData[0]:signupData
