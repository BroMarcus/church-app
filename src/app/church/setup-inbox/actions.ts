...
export async function approveSetupPlan(formData:FormData){
 ...
 const {data:updated,error:updateError}=await supabase.from('church_setup_uploads').update({status:'ready',approved_at:new Date().toISOString(),created_record_id:createdId,created_record_type:createdType}).eq('id',id).eq('church_id',membership!.church_id).eq('status','reviewing').select('id').maybeSingle();if(updateError||!updated){console.error('approveSetupPlan status update failed',{churchId:membership!.church_id,id,createdId:createdId?String(createdId).slice(0,36):null,code:boundedCode(updateError?.code)});redirect(inbox(lang,'approve'))}revalidatePath('/church/setup-inbox');revalidatePath('/learning')
}
