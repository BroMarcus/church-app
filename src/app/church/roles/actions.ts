...
const error=(message:string):never=>redirect('/church/roles?error='+encodeURIComponent(message))
...
export async function updateRole(formData:FormData){
 ...
 const {data:existing}=await supabase.from('church_roles').select('id,permissions').eq('id',id).eq('church_id',churchId).maybeSingle();if(!existing)error('Role not found')
 const nextPermissions=permissions(formData)
...
export async function assignRole(formData:FormData){
 ...
 const {data:role}=await supabase.from('church_roles').select('id,permissions').eq('id',roleId).eq('church_id',churchId).eq('active',true).maybeSingle();if(!role)error('Role not available')
...
export async function removeRoleAssignment(formData:FormData){
 ...
 const {data:assignment}=await supabase.from('church_role_assignments').select('id,role_id').eq('id',id).eq('church_id',churchId).maybeSingle();if(!assignment)error('Assignment not found')
