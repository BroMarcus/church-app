...
export default function PhoneProofClient({lang,churchId}:{lang:Lang;churchId:string}){
      ...
      if(raw){
        ...
        for(const id of ids){
          ...
          if(value&&['untested','pass','fail'].includes(String(value.result))){
            next[id]={result:value.result as Result,device:String(value.device??'').slice(0,120),account:String(value.account??'').slice(0,120),date:String(value.date??'').slice(0,20),notes:String(value.notes??'').slice(0,500)}
          }
        }
        setState(next)
      }
  ...
  function summary(){
    const lines=[t.summaryTitle,`${t.language}: ${lang==='es'?'Español':'English'}`,`PASS: ${stats.pass}/${ids.length} | FAIL: ${stats.fail} | ${t.remaining}: ${stats.remaining}`,'']
    for(const id of ids){
      const item=state[id]
      const label=t[id]
      ...
      {ids.map((id,index)=>{
        ...
        return <article className={`proof-item ${item.result}`} key={id}>
          <div className="proof-item-head"><div><span className="step">{index+1}</span><h2>{t[id]}</h2></div><div className="result-buttons" role="group" aria-label={`${index+1}. ${t[id]}`}>
            ...
            <button type="button" className={item.result==='pass'?'active':''} onClick={()=>update(id,{result:'pass'})}>{t.pass}</button>
            <button type="button" className={item.result==='fail'?'active':''} onClick={()=>update(id,{result:'fail'})}>{t.fail}</button>
          </div></div>
