...
const copy={
  en:{
    ...
    evidenceOpen:'Open phone-proof checklist →',
  ...
  es:{
    ...
    evidenceOpen:'Abrir lista de prueba en teléfono →',
...
export default async function ReadinessPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  return <main className="shell">
    ...
    <section className="card proof-gate"><div className="proof-icon"><Smartphone size={22}/></div><div><div className="pill">{t.proofPill}</div><h2>{t.proofTitle}</h2><p>{t.proofBody}</p><strong>{t.evidenceTitle}</strong><p className="muted">{t.evidenceBody}</p><Link href={`/church/readiness/phone-proof${languageQuery}`}>{t.evidenceOpen}</Link></div></section>
