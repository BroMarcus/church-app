...
export async function HealthTrends({churchId}:{churchId:string}){
  ...
  const [{data:members},{data:milestones},{data:reports},{data:outreach}]=await Promise.all([
    ...
    supabase.from('group_reports').select('group_id,meeting_date,attendance_count,first_time_guests,prayer_needs,issues_notes').gte('meeting_date',yearAgoIso).order('meeting_date'),
  ...
  const reportRows=reports??[],outreachRows=outreach??[]
