...
export default async function ChurchAnalyticsPage(){
  ...
  const memberIds=(members??[]).map((m:any)=>m.user_id),activeSet=new Set(memberIds),groupIds=(groups??[]).map((g:any)=>g.id),courseIds=(courses??[]).map((c:any)=>c.id),teamIds=(teamAssignments??[]).filter((a:any)=>a.confirmation_required).map((a:any)=>a.id)
  ...
  const milestoneRows=(milestones??[]).filter((m:any)=>activeSet.has(m.user_id)),total=memberIds.length,verifiedRows=milestoneRows.length
  ...
  return <main className="shell">
    ...
    <section className="health-grid"><div className="card health-card"><strong>{total}</strong><span>Active members</span></div><div className="card health-card"><strong>{grouped}</strong><span>Group connected</span></div><div className="card health-card"><strong>{membersAttended30d}</strong><span>Members attended group • 30d</span></div><div className="card health-card"><strong>{learnersCompleted}</strong><span>Completed a course</span></div><div className="card health-card"><strong>{serving}</strong><span>Serving / accepted</span></div></section>
    <div className="analytics-layout">
      <section className="card analytics-panel"><div className="pill">30-DAY MINISTRY PULSE</div><h2>What Friendship Groups are reporting</h2><p className="small muted">These numbers update from submitted Friendship Group reports. They are ministry-report totals, separate from verified individual member milestones.</p><div className="stage-list"><div className="stage-row"><span>Active groups reporting</span><strong>{groupsReporting}/{groupIds.length}</strong></div><div className="stage-row"><span>Meetings reported</span><strong>{reportMeetings}</strong></div><div className="stage-row"><span>Distinct members recorded present</span><strong>{membersAttended30d}</strong></div><div className="stage-row"><span>Total attendance entries</span><strong>{reportAttendance}</strong></div><div className="stage-row"><span>First-time guests</span><strong>{reportGuests}</strong></div><div className="stage-row"><span>Active Bible studies reported</span><strong>{reportStudies}</strong></div><div className="stage-row"><span>Baptisms reported</span><strong>{reportBaptisms}</strong></div><div className="stage-row"><span>Holy Ghost received reported</span><strong>{reportHolyGhost}</strong></div></div><div className="analytics-note">Individual attendance appears only when a leader checks members present on the digital group report. Older aggregate reports remain valid.</div></section>
