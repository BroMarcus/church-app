...
export default async function LeadershipDevelopmentPage({searchParams}:{searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  const [{data:members},{data:milestones},{data:reviews}]=await Promise.all([
  ...
  const rows=(members??[]).map((member:any)=>{
    ...
    const objective=gates.slice(0,7).filter(([,ok])=>ok).length
    const ready=gates.every(([,ok])=>ok)
    return {member,m,review,gates,objective,ready,name:personName(pm.get(member.user_id))}
  }).sort((a,b)=>Number(b.ready)-Number(a.ready)||b.objective-a.objective||a.name.localeCompare(b.name))

  const readyCount=rows.filter(r=>r.ready).length
  const inTimothys=rows.filter(r=>r.m.timothys_status==='in_progress').length
  const needsReview=rows.filter(r=>r.objective>=5&&(r.review.faithfulness_status!=='faithful'||r.review.pastoral_approval_status!=='approved')).length
  ...
  return <main className="shell">
    ...
    <section className="admin-hero card"><div><div className="pill">LEADERSHIP PIPELINE</div><h1>Develop the next leaders.</h1><p className="muted">Training can be measured. Faithfulness and readiness for leadership remain pastoral decisions.</p></div><div className="admin-badge"><Users size={22}/><div><strong>{rows.length}</strong><span>people in view</span></div></div></section>
    ...
    <section className="stat-grid"><div className="card stat-card"><CheckCircle2/><div><strong>{readyCount}</strong><span>Ready for leadership</span></div></div><div className="card stat-card"><Users/><div><strong>{inTimothys}</strong><span>Timothys in progress</span></div></div><div className="card stat-card"><ShieldCheck/><div><strong>{needsReview}</strong><span>Need pastoral review</span></div></div><div className="card stat-card"><Circle/><div><strong>{rows.filter(r=>r.objective<5).length}</strong><span>Still developing</span></div></div></section>

    <section className="card admin-note"><div className="pill">FRIENDSHIP GROUP LEADER PATH</div><h3>Current readiness standard</h3><p className="muted">Active membership, baptism, Holy Ghost, First Steps, Effective Soul Winning, Timothys, current covenant, faithfulness review, and pastoral approval. This can be adjusted later when New Life finalizes the exact leadership standard.</p></section>

    <div className="section-heading"><div><div className="pill">PEOPLE</div><h2>Leadership development</h2></div><span className="small muted">Objective progress + pastoral review</span></div>
    <section className="member-list">{rows.map(r=><article className="card" key={r.member.user_id} style={{padding:18}}>
      <div className="row" style={{justifyContent:'space-between',alignItems:'flex-start',gap:18,flexWrap:'wrap'}}><div style={{minWidth:230,flex:'1 1 260px'}}><div className="row"><div className="avatar large">{r.name.slice(0,1).toUpperCase()}</div><div><h3 style={{margin:'0 0 4px'}}>{r.name}</h3><span className="small muted">{r.member.role.replaceAll('_',' ')}</span></div></div><div style={{marginTop:14,display:'flex',flexWrap:'wrap',gap:7}}>{r.gates.map(([label,ok])=><span key={label} style={{fontSize:11,padding:'6px 8px',borderRadius:999,border:'1px solid var(--line)',background:ok?'#173324':'#211730',color:ok?'#c9f5d7':'#d8cce2'}}>{ok?'✓':'○'} {label}</span>)}</div><p className="small muted" style={{marginBottom:0}}>{r.ready?'Ready for leadership review/assignment.':`${r.objective}/7 objective requirements complete.`}</p></div>
