...
const returnBase=(formData:FormData,userId:string,l:'en'|'es')=>{
  const candidate=text(formData,'return_to')
  if(candidate){
    try{
      const url=new URL(candidate,'https://kingdom.local')
      if(url.origin==='https://kingdom.local'&&url.pathname==='/church/member-control'&&url.searchParams.get('member')===userId)return `${url.pathname}${url.search}`
    }catch{}
  }
  return `/church/members/${userId}?lang=${l}`
}
const back=(formData:FormData,userId:string,l:'en'|'es',key:string,value='1')=>{const base=returnBase(formData,userId,l);return `${base}${base.includes('?')?'&':'?'}${key}=${encodeURIComponent(value)}`}
...
export async function updateMemberAccessFromRecord(formData:FormData){
  ...
  if(!churchId||!targetUserId||!membershipId||!roles.includes(role as any)||!membershipStatuses.includes(status as any))redirect(back(formData,targetUserId,l,'error',l==='es'?'Cambio de acceso inválido.':'Invalid access change.'))
  const {supabase,actorId,actor,target}=await requireAdmin(churchId,targetUserId)
  if(target.id!==membershipId)redirect(back(formData,targetUserId,l,'error',l==='es'?'Registro de membresía inválido.':'Invalid membership record.'))
  if(actor.role!=='pastor'&&(target.role==='pastor'||role==='pastor'))redirect(back(formData,targetUserId,l,'error',l==='es'?'Solo un pastor puede asignar o cambiar el rol de pastor.':'Only a pastor can assign or change the Pastor role.'))
  const removingAuthority=status!=='active'||!['pastor','church_admin'].includes(role)
  if(targetUserId===actorId&&removingAuthority)redirect(back(formData,targetUserId,l,'error',l==='es'?'No puedes quitar tu propio acceso administrativo.':'You cannot remove your own admin access.'))
  if(['pastor','church_admin'].includes(target.role)&&target.status==='active'&&removingAuthority){
    ...
    if((count??0)<1)redirect(back(formData,targetUserId,l,'error',l==='es'?'La iglesia debe conservar al menos un pastor o administrador activo.':'The church must keep at least one active Pastor or Church Admin.'))
  }
  const {error}=await supabase.from('church_memberships').update({role,status}).eq('id',membershipId)
  if(error){console.error('updateMemberAccessFromRecord failed',{code:error.code,message:error.message});redirect(back(formData,targetUserId,l,'error',saveError(l,'No pudimos guardar el acceso del miembro.','We could not save member access.')))}
  revalidatePath(`/church/members/${targetUserId}`);revalidatePath('/church/member-control');revalidatePath('/church');revalidatePath('/');
  redirect(back(formData,targetUserId,l,'access'))
}
...
export async function updateMemberProfileFromRecord(formData:FormData){
  ...
  if(profileError){console.error('updateMemberProfileFromRecord profile failed',{code:profileError.code,message:profileError.message});redirect(back(formData,targetUserId,l,'error',saveError(l,'No pudimos guardar el perfil del miembro.','We could not save the member profile.')))}
  const privatePayload={user_id:targetUserId,email:emptyNull(clean(text(formData,'email'),180).toLowerCase()),phone:emptyNull(clean(text(formData,'phone'),40)),address_line1:emptyNull(clean(text(formData,'address_line1'),180)),address_line2:emptyNull(clean(text(formData,'address_line2'),180)),city:emptyNull(clean(text(formData,'city'),100)),state:emptyNull(clean(text(formData,'state'),60)),postal_code:emptyNull(clean(text(formData,'postal_code'),20)),birthday:emptyNull(text(formData,'birthday')),marriage_anniversary:emptyNull(text(formData,'marriage_anniversary'))}
  const {error:detailsError}=await supabase.from('member_private_details').upsert(privatePayload,{onConflict:'user_id'})
  if(detailsError){console.error('updateMemberProfileFromRecord private details failed',{code:detailsError.code,message:detailsError.message});redirect(back(formData,targetUserId,l,'error',saveError(l,'El perfil público se guardó, pero no pudimos guardar los datos privados.','The public profile saved, but we could not save the private details.')))}
  revalidatePath(`/church/members/${targetUserId}`);revalidatePath('/church/member-control');revalidatePath('/church');revalidatePath('/directory');
  redirect(back(formData,targetUserId,l,'profile'))
}
...
export async function overrideCourseEnrollment(formData:FormData){
  ...
  if(!churchId||!targetUserId||!courseId)redirect(back(formData,targetUserId,l,'error',l==='es'?'Falta el curso o miembro.':'Missing course or member.'))
  ...
  if(!course)redirect(back(formData,targetUserId,l,'error',l==='es'?'El curso no pertenece a esta iglesia.':'Course does not belong to this church.'))
  const reason=clean(text(formData,'override_reason'),500)
  if(reason.length<3)redirect(back(formData,targetUserId,l,'error',l==='es'?'Escribe una razón para la corrección manual.':'Enter a reason for the manual correction.'))
  ...
  if(error){console.error('overrideCourseEnrollment failed',{code:error.code,message:error.message});redirect(back(formData,targetUserId,l,'error',saveError(l,'No pudimos guardar la corrección del curso.','We could not save the course correction.')))}
  revalidatePath(`/church/members/${targetUserId}`);revalidatePath('/church/member-control');revalidatePath('/learning');revalidatePath('/journey');
  redirect(back(formData,targetUserId,l,'course'))
}
...
export async function updateMemberMinistryApplication(formData:FormData){
  ...
  if(!churchId||!targetUserId||!applicationId||!ministryStatuses.includes(status as any))redirect(back(formData,targetUserId,l,'error',l==='es'?'Cambio de ministerio inválido.':'Invalid ministry change.'))
  const {supabase,actorId}=await requireAdmin(churchId,targetUserId)
  ...
  if(!application||ministry?.church_id!==churchId)redirect(back(formData,targetUserId,l,'error',l==='es'?'Solicitud de ministerio no encontrada.':'Ministry application not found.'))
  ...
  if(error){console.error('updateMemberMinistryApplication failed',{code:error.code,message:error.message});redirect(back(formData,targetUserId,l,'error',saveError(l,'No pudimos guardar el cambio de ministerio.','We could not save the ministry change.')))}
  revalidatePath(`/church/members/${targetUserId}`);revalidatePath('/church/member-control');revalidatePath('/serve');revalidatePath('/teams');
  redirect(back(formData,targetUserId,l,'service'))
}
...
export async function updateMemberBusinessFromRecord(formData:FormData){
  ...
  if(!churchId||!targetUserId||!businessId||!businessStatuses.includes(status as any))redirect(back(formData,targetUserId,l,'error',l==='es'?'Cambio de negocio inválido.':'Invalid business change.'))
  const {supabase}=await requireAdmin(churchId,targetUserId)
  ...
  if(!listing||listing.church_id!==churchId||listing.owner_user_id!==targetUserId)redirect(back(formData,targetUserId,l,'error',l==='es'?'Negocio no encontrado para este miembro.':'Business not found for this member.'))
  ...
  if(error){console.error('updateMemberBusinessFromRecord failed',{code:error.code,message:error.message});redirect(back(formData,targetUserId,l,'error',saveError(l,'No pudimos guardar el negocio del miembro.','We could not save the member business.')))}
  revalidatePath(`/church/members/${targetUserId}`);revalidatePath('/church/member-control');revalidatePath('/business');
  redirect(back(formData,targetUserId,l,'business'))
}
