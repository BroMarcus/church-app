...
export default async function MemberRecordPage({params,searchParams}:{params:Promise<{userId:string}>;searchParams:Promise<{saved?:string;practicum?:string;error?:string;lang?:string}>}){
  ...
  if(!membership)redirect(`/church?lang=${lang}&error=`+encodeURIComponent(es?'Miembro no encontrado en esta iglesia.':'Member not found in this church.'))
  ...
  return <main className="shell">
    ...
    <form action={updateMilestones} className="record-form">
      ...
      <section className="card record-section"><div className="record-section-head"><div><Sparkles/><div><h2>{es?'Nuevo Nacimiento':'New Birth'}</h2><p>{es?'Hitos de salvación verificados por el liderazgo. Registra una fecha exacta, aproximada o desconocida sin inventarla.':'Leadership-verified salvation milestones. Record an exact, approximate, or unknown date without guessing.'}</p></div></div><span className="verified-label"><ShieldCheck size={14}/> {es?'Verificado':'Verified'}</span></div><div className="record-grid"><SelectField label={es?'Recibió el Espíritu Santo':'Holy Ghost received'} name="holy_ghost_received" value={boolValue(m.holy_ghost_received)} options={yesNo[lang]}/><DateField label={es?'Fecha del Espíritu Santo':'Holy Ghost date'} name="holy_ghost_date" value={m.holy_ghost_date}/><SelectField label={es?'Precisión de la fecha':'Date accuracy'} name="holy_ghost_date_precision" value={m.holy_ghost_date_precision??'unknown'} options={precision[lang]}/><SelectField label={es?'Bautizado':'Baptized'} name="baptized" value={boolValue(m.baptized)} options={yesNo[lang]}/><DateField label={es?'Fecha de bautismo':'Baptism date'} name="baptism_date" value={m.baptism_date}/><SelectField label={es?'Precisión de la fecha':'Date accuracy'} name="baptism_date_precision" value={m.baptism_date_precision??'unknown'} options={precision[lang]}/></div></section>
      ...
      <div className="record-save card"><div><CheckCircle2/><div><strong>{es?'Guardar registro verificado':'Save verified record'}</strong><span>{es?'Los cambios están restringidos a pastores y administradores de iglesia.':'Changes are restricted to pastors and church admins.'}</span></div></div><button className="btn" type="submit">{es?'Guardar hitos':'Save milestones'}</button></div>
    </form></div>
  </main>
}
