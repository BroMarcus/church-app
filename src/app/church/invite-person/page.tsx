...
export default async function InvitePersonPage({searchParams}:{searchParams:Promise<{lang?:string;created?:string;status?:string}>}){
  ...
  {notice&&<div className={`notice ${params.status==='created'&&invite?'success':'error'}`} role={params.status==='created'&&invite?'status':'alert'} aria-live="polite">{params.status==='created'&&!invite?(es?'La invitación ya no está abierta para compartir. Revisa Invitaciones y usa solamente el enlace abierto más reciente.':'That invitation is no longer open to share. Review Invitations and use only the newest open link.'):notice}</div>}
  {createdInviteClosed&&<div className="notice error" role="alert">{es?'Este enlace fue usado, revocado o venció. No lo vuelvas a enviar. Abre Invitaciones y comparte solamente la invitación abierta más reciente.':'This link was used, revoked, or expired. Do not resend it. Open Invitations and share only the newest open invitation.'} <Link href={l('/church/invites')}>{es?'Ver invitaciones abiertas':'View open invitations'}</Link></div>}
  {hasCreatedParam&&!invite&&!createdInviteClosed&&<div className="notice error" role="alert">{es?'No pudimos volver a cargar esa invitación. No crees otra todavía; abre Invitaciones para revisar primero.':'We could not reload that invitation. Do not create another one yet; open Invitations and check first.'} <Link href={l('/church/invites')}>{es?'Revisar invitaciones':'Review invitations'}</Link></div>}
  {invite&&<section className="card" style={{padding:20,marginBottom:18,border:'1px solid rgba(34,197,94,.35)'}}><div className="pill">{es?'INVITACIÓN LISTA':'INVITATION READY'}</div><h2>{[invite.first_name,invite.last_name].filter(Boolean).join(' ')||invite.email}</h2><div className="small muted">{invite.email}{invite.phone?` • ${invite.phone}`:''} • {es?'vence':'expires'} ${new Date(invite.expires_at).toLocaleString()}</div><div className="small muted" style={{wordBreak:'break-all',margin:'12px 0'}}>{inviteUrl}</div><CopyKnownInvite url={inviteUrl} label={es?'Copiar invitación':'Copy invitation'}/><p className="small muted">{es?'Antes de enviarla, confirma que esta siga siendo la invitación abierta más reciente. Si haces otra, envía solamente el nuevo enlace y pide que ignore los mensajes anteriores.':'Before sending it, confirm this is still the newest open invitation. If you replace it, send only the new link and ask them to ignore older messages.'}</p></section>}
