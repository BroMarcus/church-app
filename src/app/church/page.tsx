...
import { Church,MailPlus,ShieldCheck,UserCheck,Users } from 'lucide-react'
...
export default async function ChurchAdminPage({searchParams}:{searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  const [{data:memberships},{count:openInvites}]=await Promise.all([
    supabase.from('church_memberships').select('id,user_id,role,status,joined_at,created_at').eq('church_id',actor.church_id).order('created_at',{ascending:true}),
    supabase.from('church_invites').select('*',{count:'exact',head:true}).eq('church_id',actor.church_id).is('redeemed_at',null).is('revoked_at',null).gt('expires_at',new Date().toISOString())
  ])
