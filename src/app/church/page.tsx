...
export default async function ChurchAdminPage({searchParams}:{searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  return <main className="shell">
    ...
    <section className="admin-hero card"><div><div className="pill">CHURCH ADMIN</div><h1>{church?.name??'Church Directory'}</h1><p className="muted">Manage membership, leadership access and member status.</p></div><div className="admin-badge"><ShieldCheck size={22}/><div><strong>{actor.role.replaceAll('_',' ')}</strong><span>Your access</span></div></div></section>
    ...
    <section className="member-list">{rows.map(({membership,profile,details}:any)=>{const name=profile?.display_name||[profile?.first_name,profile?.last_name].filter(Boolean).join(' ')||'Unnamed member';const isYou=membership.user_id===userId;return <article className="card member-admin-card" key={membership.id}><div className="member-main"><div className="avatar large">{name.slice(0,1).toUpperCase()}</div><div className="member-copy"><div className="member-name"><strong>{name}</strong>{isYou&&<span className="mini-pill">YOU</span>}</div><span>{details?.email??'Email not available'}</span><small>{details?.phone||'No phone added'} • Joined {niceDate(membership.joined_at)}</small></div></div><form action={updateMembership} className="member-controls"><input type="hidden" name="membership_id" value={membership.id}/><label><span>Role</span><select name="role" defaultValue={membership.role}>{roleOptions.map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></label><label><span>Status</span><select name="status" defaultValue={membership.status}>{statusOptions.map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></label><button className="btn" type="submit">Save</button></form></article>})}{!rows.length&&<div className="card empty"><h3>No members yet.</h3><p className="muted">New Alpha signups will appear here automatically.</p></div>}</section>
    <section className="card admin-note"><div className="pill">NEXT ADMIN LAYER</div><h3>Verified discipleship milestones</h3><p className="muted">Next: Holy Ghost, baptism, First Steps, ministry training and Bible-study qualification with leadership verification.</p></section>
  </main>
}
