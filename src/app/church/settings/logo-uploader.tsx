...
export function LogoUploader({churchId,currentPath}:{churchId:string;currentPath?:string|null}){
  ...
  async function upload(file:File){
    ...
    setMessage('Church logo updated.');setBusy(false);router.refresh()
  }
  return <div className="logo-uploader"><label className="ghost" style={{cursor:'pointer'}}><ImageUp size={14}/>{busy?' Uploading…':' Upload church logo'}<input hidden type="file" accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={e=>{const f=e.target.files?.[0];if(f)upload(f)}}/></label>{message&&<span className="small muted">{message}</span>}</div>
}
