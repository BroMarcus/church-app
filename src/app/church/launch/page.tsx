...
const copy={
  en:{admin:'← Church Admin',title:'Set up one thing at a time.',subtitle:'Only do the card marked “Do this next.” Everything else can wait until you are ready.',next:'DO THIS NEXT',open:'Open this step →',ready:'pilot essentials ready',of:'of',done:'Done',needed:'Still needed',final:'Final pilot check',allSteps:'See all pilot setup steps',safety:'Pilot safety reminders',optional:'OPTIONAL FOR NOW',optionalTitle:'Branding and church materials',optionalBody:'These improve the experience, but they should not stop you from testing with real people.',branding:'Logo, color & welcome message',brandingBody:'Add your church look and welcome message.',uploads:'Upload church materials',uploadsBody:'Bring in manuals, forms, curriculum, policies, logos and calendars through one setup inbox.',pilot:'PILOT HELP',handles:'Testing help when you need it',small:'Start with real people',smallBody:'A small pilot is enough. Make sure they can create an account, confirm email, log in, use Start Here and recover a forgotten password.',help:'Need help?',helpBody:'Kingdom Guide gives plain-language directions and points you to the right screen.',guide:'Open Kingdom Guide →',when:'WHEN THE ESSENTIALS ARE DONE',run:'Run the pilot from a phone.',runBody:'Test signup, email confirmation, login, password recovery, Start Here, Home, Profile, Learning, Groups, Calendar, Prayer/Private Care and Feedback before expanding.',english:'English',spanish:'Español',church:'Your Church',accounts:'Protect existing accounts',accountsBody:'If someone already has a Kingdom Network account, have them log in first and then use the church join link. Do not create a second account for the same person.',join:'Open Join Center →',password:'Practice password recovery once',passwordBody:'Before the pilot grows, have one trusted pilot user confirm they can reset a forgotten password from the login screen.',signin:'Open login →',pendingInvites:'pending invitation(s)',joinedRule:'This turns ready only after a real non-admin pilot member has joined.'},
  es:{admin:'← Administración',title:'Configura una sola cosa a la vez.',subtitle:'Haz solamente la tarjeta que dice “Haz esto ahora”. Todo lo demás puede esperar hasta que estés listo.',next:'HAZ ESTO AHORA',open:'Abrir este paso →',ready:'puntos esenciales listos',of:'de',done:'Listo',needed:'Falta',final:'Revisión final del piloto',allSteps:'Ver todos los pasos del piloto',safety:'Recordatorios de seguridad del piloto',optional:'OPCIONAL POR AHORA',optionalTitle:'Diseño y materiales de la iglesia',optionalBody:'Estas cosas mejoran la experiencia, pero no deben impedir que pruebes la plataforma con personas reales.',branding:'Logo, color y mensaje de bienvenida',brandingBody:'Agrega la apariencia de tu iglesia y un mensaje de bienvenida.',uploads:'Sube materiales de la iglesia',uploadsBody:'Agrega manuales, formularios, currículo, políticas, logos y calendarios desde una sola bandeja.',pilot:'AYUDA DEL PILOTO',handles:'Ayuda para probar cuando la necesites',small:'Empieza con personas reales',smallBody:'Un piloto pequeño es suficiente. Confirma que puedan crear una cuenta, confirmar el correo, iniciar sesión, usar Empieza Aquí y recuperar una contraseña olvidada.',help:'¿Necesitas ayuda?',helpBody:'Kingdom Guide da instrucciones sencillas y te lleva a la pantalla correcta.',guide:'Abrir Kingdom Guide →',when:'CUANDO LOS PUNTOS ESENCIALES ESTÉN LISTOS',run:'Prueba el piloto desde un teléfono.',runBody:'Prueba registro, confirmación por correo, inicio de sesión, recuperación de contraseña, Empieza Aquí, Inicio, Perfil, Aprendizaje, Grupos, Calendario, Oración/Cuidado Privado y Comentarios antes de expandir.',english:'English',spanish:'Español',church:'Tu Iglesia',accounts:'Protege las cuentas existentes',accountsBody:'Si alguien ya tiene una cuenta de Kingdom Network, pídele que primero inicie sesión y después use el enlace para unirse a la iglesia. No crees una segunda cuenta para la misma persona.',join:'Abrir Centro para Unirse →',password:'Prueba la recuperación de contraseña una vez',passwordBody:'Antes de ampliar el piloto, pide a una persona de confianza que confirme que puede cambiar una contraseña olvidada desde la pantalla de inicio de sesión.',signin:'Abrir inicio de sesión →',pendingInvites:'invitación(es) pendiente(s)',joinedRule:'Esto solo queda listo cuando un miembro real del piloto, que no sea administrador, haya entrado.'}
} as const
...
export default async function ChurchLaunchPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  <details className="card launch-optional"><summary>{t.allSteps}</summary><div className="launch-grid" style={{marginTop:14}}>{steps.map((s,index)=>{const Icon=s.Icon;return <Link href={s.href} className={`card launch-step ${s.done?'done':''}`} key={s.title}><div className="launch-num">{s.done?<Check size={14}/>:index+1}</div><div className="launch-copy"><strong><Icon size={12}/> {s.title}</strong><span>{s.body}</span>{index===2&&!people?<div className="launch-meta">{openInvites} {t.pendingInvites}</div>:null}<div className="launch-status">{index===6?t.final:s.done?t.done:t.needed}</div></div></Link>})}</div></details>
  <details className="card launch-optional"><summary>{t.safety}</summary><section className="launch-support-grid" style={{marginTop:14}}><article className="card launch-support"><ShieldCheck size={20}/><div><h3>{t.accounts}</h3><p>{t.accountsBody}</p><Link href={l('/church/join-center')}>{t.join}</Link></div></article><article className="card launch-support"><KeyRound size={20}/><div><h3>{t.password}</h3><p>{t.passwordBody}</p><Link href={l('/login?mode=signin')}>{t.signin}</Link></div></article></section></details>
  ...
  <details className="card launch-optional"><summary>{t.pilot}: {t.handles}</summary><div className="manual-grid" style={{marginTop:14}}><article className="card manual-launch-card"><h3>{t.small}</h3><p>{t.smallBody}</p></article><article className="card manual-launch-card"><h3>{t.help}</h3><p>{t.helpBody}</p><Link href={l('/guide')}>{t.guide}</Link></article></div><div className="card launch-footer" style={{marginTop:14}}><div className="pill">{t.when}</div><h2>{t.run}</h2><p className="small muted">{t.runBody}</p></div></details></main>
}