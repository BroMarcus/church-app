...
export default async function ChurchLaunchPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const materials=(setupFiles??0)>0
  const steps=lang==='es'?[
    ...
    {title:'3. Agrega a las primeras personas',body:'Haz que por lo menos una persona real del piloto entre o reciba una invitación.',href:l('/church/invites'),done:people,Icon:MailPlus},
    ...
    {title:'3. Add your first people',body:'Get at least one real pilot person in or invited.',href:'/church/invites',done:people,Icon:MailPlus},
  ...
  <section className="launch-hero card"><div><div className="pill">CHURCH BUILDER</div><h1>{t.title}</h1><p className="muted">{t.subtitle}</p></div><div className="launch-progress"><strong>{pct}%</strong><span>{completed} {t.of} {essentials.length} {t.ready}</span><div className="launch-bar"><i style={{width:`${pct}%`}}/></div></div></section>
