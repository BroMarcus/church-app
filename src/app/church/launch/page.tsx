...
export default async function ChurchLaunchPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  <section className="card launch-next"><div className="pill">{t.next}</div><h2><NextIcon size={18}/> {nextStep.title}</h2><p className="muted">{nextStep.body}</p>{nextStep===steps[2]&&!people?<p className="small launch-note">{openInvites??0} {t.pendingInvites}. {t.joinedRule}</p>:null}<Link className="btn" href={nextStep.href}>{t.open}</Link></section>
  <section className="launch-support-grid"><article className="card launch-support"><ShieldCheck size={20}/><div><h3>{t.accounts}</h3><p>{t.accountsBody}</p><Link href={l('/church/join-center')}>{t.join}</Link></div></article><article className="card launch-support"><KeyRound size={20}/><div><h3>{t.password}</h3><p>{t.passwordBody}</p><Link href={l('/login')}>{t.signin}</Link></div></article></section>
  <section className="launch-grid">{steps.map((s,index)=>{const Icon=s.Icon;return <Link href={s.href} className={`card launch-step ${s.done?'done':''}`} key={s.title}><div className="launch-num">{s.done?<Check size={14}/>:index+1}</div><div className="launch-copy"><strong><Icon size={12}/> {s.title}</strong><span>{s.body}</span>{index===2&&!people?<div className="launch-meta">{openInvites??0} {t.pendingInvites}</div>:null}<div className="launch-status">{index===6?t.final:s.done?t.done:t.needed}</div></div></Link>})}</section>
