...
export default async function JoinCenterPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  return <main className="shell">
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:16,marginBottom:18}}>
      <section className="card" style={{padding:22}}><div className="pill"><QrCode size={11}/> {es?'QR DE NUEVA VIDA':'NEW LIFE QR'}</div><div style={{marginTop:18}}><JoinQr url={joinUrl} label={es?'Escanea para unirte a Kingdom Network en esta iglesia.':'Scan to join Kingdom Network at this church.'}/></div><div className="row" style={{justifyContent:'center',gap:8,flexWrap:'wrap',marginTop:16}}><CopyJoinLink url={joinUrl} label={es?'Copiar enlace':'Copy join link'}/><Link className="ghost" href={`/join/${slug}`} target="_blank"><ExternalLink size={13}/> {es?'Vista previa':'Preview'}</Link></div></section>
