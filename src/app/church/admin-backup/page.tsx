...
const copy={
  en:{title:'Add a backup admin',subtitle:'Choose one trusted active member. They will be able to help manage Kingdom Network if the main admin is unavailable.',back:'← Church Builder',admin:'Church Admin',ready:'Backup admin is already covered',readyBody:'Your church already has at least two active pastor/admin accounts.',choose:'Choose a trusted person',chooseBody:'Only active members who are not already pastors or church admins are shown here.',make:'Make church admin',none:'No eligible active members yet.',noneBody:'Invite the trusted leader first. After they create and confirm their account, come back here and promote them.',invite:'Invite a trusted leader →',saved:'Backup admin added successfully.',warning:'Church admins can manage members, settings, invitations and other sensitive church operations. Only choose someone you fully trust.',confirm:'I understand this person will receive full church-admin access.',home:'Home',step:'STEP 2',important:'IMPORTANT',readyPill:'READY',countLabel:'pastor/admin'},
  es:{title:'Agrega un administrador de respaldo',subtitle:'Elige a un miembro activo de confianza. Podrá ayudar a administrar Kingdom Network si el administrador principal no está disponible.',back:'← Church Builder',admin:'Administración',ready:'El administrador de respaldo ya está cubierto',readyBody:'Tu iglesia ya tiene por lo menos dos cuentas activas de pastor/administrador.',choose:'Elige a una persona de confianza',chooseBody:'Aquí solo aparecen miembros activos que todavía no son pastores ni administradores de la iglesia.',make:'Hacer administrador de la iglesia',none:'Todavía no hay miembros activos elegibles.',noneBody:'Primero invita al líder de confianza. Después de que cree y confirme su cuenta, vuelve aquí para darle acceso administrativo.',invite:'Invitar a un líder de confianza →',saved:'Administrador de respaldo agregado correctamente.',warning:'Los administradores pueden manejar miembros, ajustes, invitaciones y otras operaciones sensibles de la iglesia. Elige solamente a alguien de plena confianza.',confirm:'Entiendo que esta persona recibirá acceso completo de administrador de la iglesia.',home:'Inicio',step:'PASO 2',important:'IMPORTANTE',readyPill:'LISTO',countLabel:'pastor/admin'}
} as const
...
export default async function BackupAdminPage({searchParams}:{searchParams:Promise<{lang?:string;saved?:string;error?:string}>}){
  ...
  const [{data:memberships},{count:adminCount}]=await Promise.all([
    supabase.from('church_memberships').select('id,user_id,role,status').eq('church_id',churchId).eq('status','active').not('user_id','eq',userId).not('role','in','("pastor","church_admin")').order('created_at',{ascending:true}),
  ...
  return <main className="shell">
    ...
    <section className="admin-hero card"><div><div className="pill">CHURCH BUILDER</div><h1>{t.title}</h1><p className="muted">{t.subtitle}</p></div><div className="admin-badge"><ShieldCheck size={22}/><div><strong>{adminCount??0}</strong><span>{t.countLabel}</span></div></div></section>
    ...
    {ready?<section className="card admin-note"><div className="pill">{t.readyPill}</div><h2>{t.ready}</h2><p className="muted">{t.readyBody}</p><Link className="btn" href={l('/church/launch')}>{t.back}</Link></section>:<>
      <section className="card admin-note"><div className="pill">{t.important}</div><p>{t.warning}</p></section>
      <div className="section-heading"><div><div className="pill">{t.step}</div><h2>{t.choose}</h2></div><span className="small muted">{t.chooseBody}</span></div>
      <section className="member-list">{eligible.map(({membership,profile,details}:any)=>{const name=profile?.display_name||[profile?.first_name,profile?.last_name].filter(Boolean).join(' ')||details?.email||'Member';return <article className="card member-admin-card" key={membership.id}><div className="member-main"><div className="avatar large">{name.slice(0,1).toUpperCase()}</div><div className="member-copy"><div className="member-name"><strong>{name}</strong></div><span>{details?.email??''}</span><small>{String(membership.role).replaceAll('_',' ')}</small></div></div><form action={promoteBackupAdmin} className="member-controls"><input type="hidden" name="membership_id" value={membership.id}/><input type="hidden" name="lang" value={lang}/><label className="small" style={{display:'flex',alignItems:'flex-start',gap:8,maxWidth:360}}><input type="checkbox" name="confirm_admin" value="yes" required style={{marginTop:3}}/><span>{t.confirm}</span></label><button className="btn" type="submit"><UserPlus size={15}/> {t.make}</button></form></article>})}{!eligible.length&&<div className="card empty"><Users size={28}/><h3>{t.none}</h3><p className="muted">{t.noneBody}</p><Link className="btn" href={l('/church/invites')}>{t.invite}</Link></div>}</section>
    </>}
  </main>
}
