...
type Query={lang?:string;member_added?:string;member_saved?:string;member_removed?:string;error?:string}
...
export default async function GroupRosterPage({params,searchParams}:{params:Promise<{groupId:string}>;searchParams:Promise<Query>}){
  ...
  return <main className="shell">
    ...
    {query.member_added&&<div className="notice success">{t('Member added to the roster.','Miembro agregado a la lista.')}</div>}{query.member_saved&&<div className="notice success">{t('Roster change saved.','Cambio de lista guardado.')}</div>}{query.member_removed&&<div className="notice success">{t('Member removed from this group. Their church account and history were not deleted.','Miembro quitado de este grupo. Su cuenta e historial de la iglesia no fueron borrados.')}</div>}{query.error&&<div className="notice error">{query.error}</div>}
