...
export default async function GroupDetailPage({params,searchParams}:{params:Promise<{groupId:string}>;searchParams:Promise<{error?:string;reported?:string;attendance_recorded?:string;guests_added?:string;guest_duplicates?:string;milestones_queued?:string;member?:string;details?:string;private?:string;requested?:string;cancelled?:string;reviewed?:string}>}){
  ...
  return <main className="shell">
    ...
    <div className="group-layout"><div>
      ...
      {canReport&&<section className="card group-section group-report"><div className="pill">LEADER REPORT</div><h2>Submit meeting report</h2><p className="small muted">One report should capture attendance, guests, spiritual milestones, prayer needs and anything leadership needs to follow up.</p><form action={submitGroupReport}><input type="hidden" name="group_id" value={groupId}/><div className="report-grid"><label className="field"><span>Meeting date</span><input type="date" name="meeting_date" required/></label><label className="field"><span>Meeting type</span><select name="meeting_type" defaultValue="regular"><option value="regular">Regular group</option><option value="outreach">Outreach</option><option value="fellowship">Fellowship</option><option value="special">Special event</option></select></label><label className="field"><span>Location</span><input name="location_label" defaultValue={group.location_label??''} placeholder="Home, church, park…"/></label><label className="field"><span>Total attendance</span><input type="number" min="0" name="attendance_count" defaultValue="0"/></label><label className="field"><span>First-time guests</span><input type="number" min="0" name="first_time_guests" defaultValue="0"/></label><label className="field"><span>Active Bible studies</span><input type="number" min="0" name="active_bible_studies" defaultValue="0"/></label><label className="field"><span>Baptisms</span><input type="number" min="0" name="baptisms" defaultValue="0"/></label><label className="field"><span>Holy Ghost received</span><input type="number" min="0" name="holy_ghost_received" defaultValue="0"/></label></div>
        <div className="card" style={{padding:14,margin:'14px 0',background:'rgba(255,255,255,.03)'}}><div className="pill">MEMBER ATTENDANCE</div><h3 style={{margin:'8px 0 4px'}}>Who was there?</h3><p className="small muted" style={{marginTop:0}}>Check the members who attended. Kingdom Network stores this separately from the total so future health reports can show attendance patterns without guessing.</p><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:8}}>{roster.map((r:any)=>{const name=personName(pm.get(r.user_id));return <label className="checkrow" key={r.user_id}><input type="checkbox" name="attendee_user_id" value={r.user_id}/><span>{name}</span></label>})}</div></div>
        ...
        <label className="field"><span>Lesson / topic</span><input name="lesson_title" placeholder="What was taught?"/></label><div className="report-grid"><label className="field"><span>Prayer needs</span><textarea name="prayer_needs" rows={3} placeholder="Prayer needs from the meeting…"/></label><label className="field"><span>Issues / pastoral attention</span><textarea name="issues_notes" rows={3} placeholder="Anything leadership needs to know or address…"/></label></div><label className="field"><span>Follow-up / next actions</span><textarea name="follow_up_notes" rows={3} placeholder="Guests to contact, Bible studies, next actions…"/></label><label className="field"><span>General notes</span><textarea name="general_notes" rows={3} placeholder="Anything else worth recording…"/></label><button className="btn">Submit report</button></form></section>}
    </div>

    <aside><section className="card group-section"><div className="pill">RECENT REPORTS</div><h2>Meeting history</h2>{canReport?<div className="report-history">{reports.map((r:any)=><article className="report-card" key={r.id}><div className="report-top"><strong>{fmtDate(r.meeting_date)}</strong><span>{r.lesson_title||'No lesson title'}</span></div><div className="small muted" style={{marginBottom:8}}>{String(r.meeting_type||'regular').replaceAll('_',' ')}{r.location_label?` • ${r.location_label}`:''}</div><div className="report-numbers"><div><strong>{r.attendance_count}</strong><span>attendance</span></div><div><strong>{r.first_time_guests}</strong><span>guests</span></div><div><strong>{r.active_bible_studies}</strong><span>studies</span></div><div><strong>{r.baptisms}</strong><span>baptisms</span></div><div><strong>{r.holy_ghost_received}</strong><span>Holy Ghost</span></div></div>{r.prayer_needs&&<div className="report-notes"><strong>Prayer:</strong> {r.prayer_needs}</div>}{r.issues_notes&&<div className="report-notes"><strong>Attention:</strong> {r.issues_notes}</div>}{r.follow_up_notes&&<div className="report-notes"><strong>Follow-up:</strong> {r.follow_up_notes}</div>}{r.general_notes&&<div className="report-notes">{r.general_notes}</div>}</article>)}{!reports.length&&<div className="group-empty">No reports submitted yet.</div>}</div>:<div className="group-empty">Meeting reports are private to group leadership and church administration.</div>}</section></aside></div>
  </main>
}
