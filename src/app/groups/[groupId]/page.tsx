...
export default async function GroupDetailPage({params,searchParams}:{params:Promise<{groupId:string}>;searchParams:Promise<{error?:string;reported?:string;guests_added?:string;guest_duplicates?:string;milestones_queued?:string;member?:string;details?:string;private?:string;requested?:string;cancelled?:string;reviewed?:string;report_q?:string;report_type?:string}>}){
  ...
  let allReports:any[]=[]
  if(canReport){const result=await supabase.from('group_reports').select('*').eq('group_id',groupId).order('meeting_date',{ascending:false}).limit(100);allReports=result.data??[]}
  const reportQ=String(query.report_q??'').trim().toLowerCase()
  const reportType=meetingTypes.includes(String(query.report_type??''))?String(query.report_type):''
  const reports=allReports.filter((r:any)=>{
    if(reportType&&r.meeting_type!==reportType)return false
    if(!reportQ)return true
    return [r.meeting_date,r.meeting_type,r.location_label,r.lesson_title,r.prayer_needs,r.issues_notes,r.follow_up_notes,r.general_notes].some(v=>String(v??'').toLowerCase().includes(reportQ))
  })
  ...
  return <main className="shell">
    ...
    <div className="group-layout"><div>
      ...
      {canReport&&<section className="card group-section group-report"><div className="pill">LEADER REPORT</div><h2>Submit meeting report</h2><p className="small muted">Use the same report for normal meetings, Matthew parties, picnics, barbecues, and other special group events.</p><form action={submitGroupReport}><input type="hidden" name="group_id" value={groupId}/><div className="report-grid"><label className="field"><span>Meeting date</span><input type="date" name="meeting_date" required/></label><label className="field"><span>Meeting / event type</span><select name="meeting_type" defaultValue="regular"><option value="regular">Regular meeting</option><option value="matthew_party">Matthew party</option><option value="picnic">Picnic</option><option value="barbecue">Barbecue</option><option value="special_event">Special event</option><option value="other">Other</option></select></label><label className="field"><span>Location</span><input name="location_label" defaultValue={group.location_label||''} placeholder="Home, park, church, neighborhood…"/></label><label className="field"><span>Attendance</span><input type="number" min="0" name="attendance_count" defaultValue="0"/></label><label className="field"><span>First-time guests</span><input type="number" min="0" name="first_time_guests" defaultValue="0"/></label><label className="field"><span>Active Bible studies</span><input type="number" min="0" name="active_bible_studies" defaultValue="0"/></label><label className="field"><span>Baptisms</span><input type="number" min="0" name="baptisms" defaultValue="0"/></label><label className="field"><span>Holy Ghost received</span><input type="number" min="0" name="holy_ghost_received" defaultValue="0"/></label></div>
        ...
        <label className="field"><span>Lesson / topic</span><input name="lesson_title" placeholder="What was taught?"/></label><div className="report-grid"><label className="field"><span>Prayer needs</span><textarea name="prayer_needs" rows={4} placeholder="Prayer requests or spiritual needs raised during the meeting…"/></label><label className="field"><span>Issues / pastoral attention</span><textarea name="issues_notes" rows={4} placeholder="Problems, conflicts, safety concerns, or items leadership should know…"/></label></div><label className="field"><span>Follow-up / next actions</span><textarea name="follow_up_notes" rows={4} placeholder="Guests to contact, Bible studies to schedule, visits, calls, next actions…"/></label><label className="field"><span>General notes</span><textarea name="general_notes" rows={3} placeholder="Anything else worth preserving with this report…"/></label><button className="btn">Submit report</button></form></section>}
    </div>

    <aside><section className="card group-section"><div className="pill">REPORT HISTORY</div><h2>Search meeting reports</h2>{canReport?<><form method="get" style={{display:'grid',gap:8,marginBottom:14}}><label className="field"><span>Search reports</span><div className="row" style={{gap:6}}><Search size={15}/><input name="report_q" defaultValue={query.report_q||''} placeholder="Date, location, lesson, prayer, issue, follow-up…"/></div></label><label className="field"><span>Event type</span><select name="report_type" defaultValue={reportType}><option value="">All types</option>{meetingTypes.map(v=><option value={v} key={v}>{meetingTypeLabel(v)}</option>)}</select></label><button className="ghost">Search</button></form><div className="small muted" style={{marginBottom:10}}>{reports.length} report{reports.length===1?'':'s'} shown • up to 100 recent reports searchable</div><div className="report-history">{reports.map((r:any)=><article className="report-card" key={r.id}><div className="report-top"><strong>{fmtDate(r.meeting_date)}</strong><span>{meetingTypeLabel(r.meeting_type||'regular')}{r.location_label?` • ${r.location_label}`:''}</span></div><div className="report-numbers"><div><strong>{r.attendance_count}</strong><span>attendance</span></div><div><strong>{r.first_time_guests}</strong><span>guests</span></div><div><strong>{r.active_bible_studies}</strong><span>studies</span></div><div><strong>{r.baptisms}</strong><span>baptisms</span></div><div><strong>{r.holy_ghost_received}</strong><span>Holy Ghost</span></div></div>{r.lesson_title&&<div className="report-notes"><strong>Lesson:</strong> {r.lesson_title}</div>}{r.prayer_needs&&<div className="report-notes"><strong>Prayer:</strong> {r.prayer_needs}</div>}{r.issues_notes&&<div className="report-notes"><strong>Issues:</strong> {r.issues_notes}</div>}{r.follow_up_notes&&<div className="report-notes"><strong>Follow-up:</strong> {r.follow_up_notes}</div>}{r.general_notes&&<div className="report-notes"><strong>Notes:</strong> {r.general_notes}</div>}</article>)}{!reports.length&&<div className="group-empty">No matching reports found.</div>}</div></>:<div className="group-empty">Meeting reports are private to group leadership and church administration.</div>}</section></aside></div>
  </main>
}
