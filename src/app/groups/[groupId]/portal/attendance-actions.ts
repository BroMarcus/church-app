...
const portalUrl=(groupId:string,tab:string,lang:string,extra:string)=>`/groups/${groupId}/portal?tab=${tab}&lang=${lang}&${extra}`
...
export async function savePortalAttendance(formData:FormData){
  ...
  const groupId=text(formData,'group_id'),meetingDate=text(formData,'meeting_date'),lang=language(formData)
  ...
  if(!canRecord)redirect(portalUrl(groupId,'attendance',lang,'error_code=attendance_access'))
  ...
  if(!requested.length)redirect(portalUrl(groupId,'attendance',lang,'error_code=attendance_empty'))
  ...
  if(!rows.length)redirect(portalUrl(groupId,'attendance',lang,'error_code=attendance_invalid'))

  const {error}=await supabase.from('group_attendance_drafts').upsert(rows,{onConflict:'group_id,user_id,meeting_date'})
  if(error){
    console.error('savePortalAttendance failed',{groupId,code:error.code})
    redirect(portalUrl(groupId,'attendance',lang,'error_code=attendance_save'))
  }

  revalidatePath(`/groups/${groupId}/portal`)
  revalidatePath(`/groups/${groupId}/roster`)
  redirect(portalUrl(groupId,'attendance',lang,'attendance_saved=1'))
}
