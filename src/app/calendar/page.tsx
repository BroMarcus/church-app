...
const fmtDate=(v:string)=>new Date(v).toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric',year:'numeric'})
const fmtTime=(v:string)=>new Date(v).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'})
...
export default async function CalendarPage({searchParams}:{searchParams:Promise<{created?:string;rsvp?:string;error?:string}>}){
  ...
  const {data:membership}=await supabase.from('church_memberships').select('church_id,role,churches(name,district_id)').eq('user_id',userId).eq('status','active').limit(1).single()
  ...
  return <main className="shell">
    <header className="topbar"><div><Link href="/" className="brand">Kingdom <span>Network</span></Link><div className="small muted">{church?.name??'Your Church'} • Calendar</div></div><Link className="ghost" href="/">← Home</Link></header>
