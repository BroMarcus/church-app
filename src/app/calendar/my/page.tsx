...
import { AlertTriangle,BookOpen,CalendarDays,Clock,MapPin,UserCheck,Users } from 'lucide-react'
...
const copy={
  en:{title:'My Schedule',subtitle:'See what you are doing, when it happens and whether you need to respond.',church:'Church Calendar',home:'Home',conflicts:'SCHEDULE CHECK',conflictTitle:'Possible schedule conflicts',conflictBody:'These items start close together. Review the details before committing to both.',assignments:'MINISTRY ASSIGNMENTS',events:'EVENTS I’M GOING TO',classes:'CLASSES',groups:'FRIENDSHIP GROUPS',noneAssignments:'No upcoming ministry assignments.',noneEvents:'No upcoming events marked Going.',noneClasses:'No upcoming classes on your schedule.',noneGroups:'You are not connected to a group yet.',call:'Call time',role:'Your role',needsResponse:'Needs response',confirmed:'Confirmed',declined:'Declined',scheduled:'Scheduled',respond:'Respond now',planning:'Personal Planning',planningBody:'Personal tasks and dates you are unavailable.'},
  es:{title:'Mi Horario',subtitle:'Mira qué vas a hacer, cuándo sucede y si necesitas responder.',church:'Calendario de la Iglesia',home:'Inicio',conflicts:'REVISIÓN DE HORARIO',conflictTitle:'Posibles conflictos de horario',conflictBody:'Estos elementos comienzan muy cerca uno del otro. Revisa los detalles antes de comprometerte con ambos.',assignments:'ASIGNACIONES DE MINISTERIO',events:'EVENTOS A LOS QUE VOY',classes:'CLASES',groups:'GRUPOS DE AMISTAD',noneAssignments:'No tienes asignaciones próximas.',noneEvents:'No tienes eventos próximos marcados como Voy a ir.',noneClasses:'No tienes clases próximas en tu horario.',noneGroups:'Todavía no estás conectado a un grupo.',call:'Hora de llegada',role:'Tu rol',needsResponse:'Necesita respuesta',confirmed:'Confirmado',declined:'No disponible',scheduled:'Programado',respond:'Responder ahora',planning:'Plan Personal',planningBody:'Tareas personales y fechas en que no estás disponible.'}
} as const
...
export default async function MySchedulePage({searchParams}:{searchParams:Promise<{lang?:string;task_created?:string;task_saved?:string;time_off?:string;error?:string}>}){
  ...
  const lang:'en'|'es'=query.lang==='es'?'es':'en',t=copy[lang]
  ...
  const [{data:assignments},{data:enrollments},{data:groupMemberships},{data:allSessions},{data:goingRsvps},{data:tasks},{data:timeOff}]=await Promise.all([
    supabase.from('team_assignments').select('id,title,starts_at,call_time,notes,ministry_id,confirmation_required,ministries(name)').eq('church_id',membership.church_id).eq('assigned_user_id',userId).gte('starts_at',nowIso).order('starts_at').limit(30),
  ...
  if(assignmentIds.length){const r=await supabase.from('team_assignment_responses').select('assignment_id,response,note').in('assignment_id',assignmentIds);assignmentResponses=r.data??[]}
  const responseBy=new Map(assignmentResponses.map((r:any)=>[r.assignment_id,r]))
  const pendingResponses=(assignments??[]).filter((a:any)=>a.confirmation_required&&!responseBy.get(a.id)).length
  ...
  return <main className="shell">
    <header className="topbar"><div><Link href={l('/')} className="brand">Kingdom <span>Network</span></Link><div className="small muted">{church?.name??(lang==='es'?'Iglesia':'Church')} • {t.title}</div></div><div className="row"><Link className="ghost" href="/calendar/my?lang=en">English</Link><Link className="ghost" href="/calendar/my?lang=es">Español</Link><Link className="ghost" href={l('/calendar')}>{t.church}</Link><Link className="ghost" href={l('/')}>{t.home}</Link></div></header>
    <section className="hero card"><div><div className="pill">{lang==='es'?'MI HORARIO':'MY SCHEDULE'}</div><h1>{t.title}</h1><p>{t.subtitle}</p>{pendingResponses>0&&<Link className="btn" href={l('/teams')} style={{marginTop:10}}><UserCheck size={14}/> {pendingResponses} {pendingResponses===1?t.needsResponse:(lang==='es'?'necesitan respuesta':'need responses')}</Link>}</div><div className="hero-stat"><strong>{(assignments?.length??0)+sessions.length+goingEvents.length+(tasks?.length??0)}</strong><span>{lang==='es'?'cosas próximas':'upcoming items'}</span></div></section>
    ...
    <div style={{display:'grid',gap:18}}>
      <section className="card" style={{padding:18}}><div className="pill">{t.assignments}</div><div style={{display:'grid',gap:10,marginTop:14}}>{(assignments??[]).map((a:any)=>{const ministry=Array.isArray(a.ministries)?a.ministries[0]:a.ministries,response:any=responseBy.get(a.id);const status=!a.confirmation_required?t.scheduled:response?.response==='confirmed'?t.confirmed:response?.response==='declined'?t.declined:t.needsResponse;const needs=a.confirmation_required&&!response;return <article key={a.id} style={{padding:'12px 0',borderBottom:'1px solid var(--line)'}}><div className="row" style={{justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}><div style={{minWidth:0,flex:'1 1 220px'}}><strong>{a.title}</strong><div className="small muted" style={{marginTop:4}}>{ministry?.name||(lang==='es'?'Ministerio':'Ministry')}{a.notes?` • ${a.notes}`:''}</div><div className="small" style={{marginTop:7,fontWeight:800}}>{status}</div>{needs&&<Link className="btn" href={l('/teams')} style={{marginTop:8}}>{t.respond}</Link>}</div><div className="small" style={{textAlign:'right',flex:'0 0 auto'}}><div><CalendarDays size={12} style={{verticalAlign:'middle'}}/> {formatChurchDate(a.starts_at,timeZone,{weekday:'short',month:'short',day:'numeric'})}</div><div><Clock size={12} style={{verticalAlign:'middle'}}/> {formatChurchTime(a.starts_at,timeZone)}</div>{a.call_time&&<div className="muted">{t.call}: {formatChurchTime(a.call_time,timeZone)}</div>}</div></div></article>})}{!assignments?.length&&<p className="muted">{t.noneAssignments}</p>}</div></section>

      <section className="card" style={{padding:18}}><div className="pill">{t.events}</div><div style={{display:'grid',gap:10,marginTop:14}}>{goingEvents.map((e:any)=><article key={e.id} style={{padding:'12px 0',borderBottom:'1px solid var(--line)'}}><div className="row" style={{justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}><div><CalendarDays size={15}/><strong style={{marginLeft:7}}>{e.title}</strong>{e.location&&<div className="small muted" style={{marginTop:4}}><MapPin size={12} style={{verticalAlign:'middle'}}/> {e.location}</div>}</div><div className="small" style={{textAlign:'right'}}><div>{formatChurchDate(e.starts_at,timeZone,{weekday:'short',month:'short',day:'numeric'})}</div><div>{formatChurchTime(e.starts_at,timeZone)}{e.ends_at?` – ${formatChurchTime(e.ends_at,timeZone)}`:''}</div></div></div></article>)}{!goingEvents.length&&<p className="muted">{t.noneEvents}</p>}</div></section>

      <section className="card" style={{padding:18}}><div className="pill">{t.classes}</div><div style={{display:'grid',gap:10,marginTop:14}}>{sessions.map((s:any)=>{const course=Array.isArray(s.courses)?s.courses[0]:s.courses;return <article key={s.id} style={{padding:'12px 0',borderBottom:'1px solid var(--line)'}}><div className="row" style={{justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}><div><BookOpen size={15}/><strong style={{marginLeft:7}}>{s.title||course?.title||(lang==='es'?'Clase':'Class')}</strong><div className="small muted" style={{marginTop:4}}>{course?.title}{s.instructor_user_id===userId?` • ${lang==='es'?'Tú enseñas':'You are teaching'}`:''}</div></div><div className="small" style={{textAlign:'right'}}><div>{niceDate(s.session_date)}</div><div>{niceTime(s.starts_at)}</div></div></div></article>})}{!sessions.length&&<p className="muted">{t.noneClasses}</p>}</div></section>

      <section className="card" style={{padding:18}}><div className="pill">{t.groups}</div><div style={{display:'grid',gap:10,marginTop:14}}>{groups.map((g:any)=><article key={g.group.id} style={{padding:'12px 0',borderBottom:'1px solid var(--line)'}}><div className="row" style={{justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}><div><Users size={15}/><strong style={{marginLeft:7}}>{g.group.name}</strong><div className="small muted" style={{marginTop:4}}>{t.role}: {g.role}{g.group.location_label?` • ${g.group.location_label}`:''}</div></div><div className="small" style={{textAlign:'right'}}><div>{g.group.meeting_day||(lang==='es'?'Por definir':'TBD')}</div><div>{niceTime(g.group.meeting_time)} • {String(g.group.meeting_frequency||'weekly').replaceAll('_',' ')}</div></div></div></article>)}{!groups.length&&<p className="muted">{t.noneGroups}</p>}</div></section>
    </div>

    <details className="card" style={{padding:18,marginTop:18}}><summary style={{cursor:'pointer',fontWeight:800}}>{t.planning}</summary><p className="small muted">{t.planningBody}</p><PersonalPlanning churchId={membership.church_id} tasks={tasks??[]} timeOff={timeOff??[]} lang={lang} timeZone={timeZone}/></details>
  </main>
}
