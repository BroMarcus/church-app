...
export default async function MySchedulePage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const [{data:assignments},{data:enrollments},{data:groupMemberships},{data:allSessions},{data:goingRsvps}]=await Promise.all([
  ...
  return <main className="shell">
    ...
    <section className="hero card"><div><div className="pill">{lang==='es'?'MI HORARIO':'MY SCHEDULE'}</div><h1>{t.title}</h1><p>{t.subtitle}</p></div><div className="hero-stat"><strong>{(assignments?.length??0)+sessions.length+goingEvents.length}</strong><span>{lang==='es'?'cosas próximas':'upcoming items'}</span></div></section>
