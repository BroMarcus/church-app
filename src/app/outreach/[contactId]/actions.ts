...
function contactHref(contactId:string,lang:'en'|'es',key?:string,value?:string){
  const params=new URLSearchParams();if(key&&value)params.set(key,value);if(lang==='es')params.set('lang','es');const q=params.toString();return `/outreach/${contactId}${q?`?${q}`:''}`
}
...
export async function createOutreachMemberInvite(formData:FormData){
  const lang=await requestLanguage(),supabase=await createClient();const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)redirect('/login')
  const contactId=text(formData,'contact_id');if(!contactId)redirect(lang==='es'?'/outreach?lang=es':'/outreach')
  ...
  const {data:existing}=await supabase.from('church_invites').select('id,outreach_contact_id,expires_at').eq('church_id',contact.church_id).eq('email',email).is('redeemed_at',null).is('revoked_at',null).order('created_at',{ascending:false}).limit(1).maybeSingle()
  if(existing?.id){
    if(existing.outreach_contact_id&&existing.outreach_contact_id!==contactId)redirect(contactHref(contactId,lang,'error',lang==='es'?'Ya existe una invitación abierta para este correo vinculada a otra persona en Evangelismo.':'An open invitation for this email is already linked to another Outreach person.'))
    const refreshNeeded=existing.outreach_contact_id!==contactId||new Date(existing.expires_at).getTime()<=Date.now()
    if(refreshNeeded){const {error}=await supabase.from('church_invites').update({outreach_contact_id:contactId,expires_at:new Date(Date.now()+7*24*60*60*1000).toISOString()}).eq('id',existing.id);if(error)redirect(contactHref(contactId,lang,'error',lang==='es'?'No se pudo preparar la invitación. Inténtelo de nuevo.':error.message))}
    revalidatePath('/church/invites');revalidatePath('/outreach');revalidatePath(`/outreach/${contactId}`);redirect(contactHref(contactId,lang,'invite',existing.id))
  }
  ...
  revalidatePath('/church/invites');revalidatePath('/outreach');revalidatePath(`/outreach/${contactId}`);redirect(contactHref(contactId,lang,'invite',invite.id))
}

export async function linkOutreachToExistingMember(formData:FormData){
  const lang=await requestLanguage(),supabase=await createClient();const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)redirect('/login')
  const contactId=text(formData,'contact_id'),memberUserId=text(formData,'member_user_id');if(!contactId||!memberUserId)redirect(lang==='es'?'/outreach?lang=es':'/outreach')
  ...
  revalidatePath('/outreach');revalidatePath(`/outreach/${contactId}`);revalidatePath('/church/analytics');revalidatePath('/journey');redirect(contactHref(contactId,lang,'linked','1'))
}
