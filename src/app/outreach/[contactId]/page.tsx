...
import { BookOpen,Compass,Link2,Mail,MapPin,Phone,ShieldCheck,UserCheck,Users } from 'lucide-react'
import { createClient } from '@/lib/supabase/server'
import { OutreachHistory } from '../outreach-history'
import { createOutreachMemberInvite,linkOutreachToExistingMember } from './actions'
...
const sourceNames:Record<string,[string,string]>={church_service:['Church service','Servicio de iglesia'],friendship_group:['Friendship Group','Grupo de Amistad'],outreach:['Outreach','Evangelismo'],event:['Event','Evento'],leader_entry:['Leader entry','Entrada de líder'],website:['Website','Sitio web'],other:['Other','Otro']}

export default async function OutreachContactPage({params,searchParams}:{params:Promise<{contactId:string}>;searchParams:Promise<{invite?:string;linked?:string;error?:string;lang?:string}>}){
  ...
  if(canInvite&&!contact.member_user_id){
    const {data:memberRows}=await supabase.from('church_memberships').select('user_id').eq('church_id',membership.church_id).eq('status','active')
    const ids=(memberRows??[]).map((m:any)=>m.user_id)
    if(ids.length){const {data:profileRows}=await supabase.from('profiles').select('id,display_name,first_name,last_name').in('id',ids);memberOptions=(profileRows??[]).map((p:any)=>({id:p.id,name:personName(p)})).sort((a,b)=>a.name.localeCompare(b.name))}
  }
  ...
  const sourceLabel=sourceNames[String(contact.source_type||'leader_entry')]?.[es?1:0]??String(contact.source_type||'leader_entry').replaceAll('_',' ')
  ...
  return <main className="shell">
    ...
    {query.linked&&<div className="notice success">{t('Outreach history linked to the existing member account.','Historial de alcance conectado a la cuenta de miembro existente.')}</div>}{query.error&&<div className="notice error">{query.error}</div>}
    ...
    <div className="outreach-layout"><section className="contact-list"><article className="card contact-card"><div className="contact-head"><div className="contact-name"><div className="avatar">{name.slice(0,1).toUpperCase()}</div><div><strong>{name}</strong><div className="small muted">{contact.email||t('No email added','Sin correo agregado')}{contact.phone?` • ${contact.phone}`:''}</div></div></div><span className="stage-chip">{stageLabel}</span></div>
      <div className="contact-meta" style={{margin:'10px 0'}}><span><MapPin size={12}/>{sourceLabel}{contact.source_label?` • ${contact.source_label}`:''}</span>{contact.source_occurred_at&&<span>{new Date(contact.source_occurred_at).toLocaleDateString()}</span>}</div>
    ...
    <aside><section className="card create-outreach"><div className="pill">{t('MEMBER CONNECTION','CONEXIÓN DE MIEMBRO')}</div>{contact.member_user_id?<><UserCheck size={24}/><h2>{t('Linked to Kingdom Network','Conectado a Kingdom Network')}</h2><p className="small muted">{t(`This Outreach history is connected to the member account for ${personName(linkedProfile)}. The Outreach record stays preserved instead of becoming a duplicate guest.`,`Este historial de alcance está conectado a la cuenta de miembro de ${personName(linkedProfile)}. El registro de alcance se conserva en lugar de crear una visita duplicada.`)}</p><Link className="btn" href={withLang(`/directory/${contact.member_user_id}`)}>{t('Open member profile','Abrir perfil del miembro')}</Link></>:canInvite?<><Link2 size={24}/><h2>{t('Connect this person to a member account','Conecte esta persona a una cuenta de miembro')}</h2><p className="small muted">{t('If this person already has a Kingdom Network account, link it here. If not, create a secure invitation. Only pastor/admin can perform this reconciliation.','Si esta persona ya tiene una cuenta de Kingdom Network, conéctela aquí. Si no, cree una invitación segura. Solo pastor/administrador puede hacer esta reconciliación.')}</p>{memberOptions.length>0&&<form action={linkOutreachToExistingMember} style={{display:'grid',gap:8,marginBottom:16}}><input type="hidden" name="contact_id" value={contactId}/><label className="field"><span>{t('Existing church member','Miembro existente')}</span><select name="member_user_id" defaultValue="" required><option value="" disabled>{t('Choose member carefully','Seleccione el miembro con cuidado')}</option>{memberOptions.map(m=><option value={m.id} key={m.id}>{m.name}</option>)}</select></label><button className="ghost"><UserCheck size={13}/> {t('Link existing member','Conectar miembro existente')}</button></form>}<div style={{borderTop:'1px solid var(--line)',paddingTop:14}}><strong>{t('No account yet?','¿Aún no tiene cuenta?')}</strong><p className="small muted">{t('The invitation is tied to this email and expires after seven days. When the person creates the account, this Outreach record links automatically.','La invitación está vinculada a este correo y vence después de siete días. Cuando la persona crea la cuenta, este registro se conecta automáticamente.')}</p>{contact.email?inviteId?<div style={{display:'grid',gap:8}}><div className="notice success"><Mail size={12}/> {t('Invitation ready for','Invitación lista para')} {contact.email}.</div><OutreachInviteLink inviteId={inviteId}/>{openInvite?.expires_at&&<span className="small muted">{t('Expires','Vence')} {new Date(openInvite.expires_at).toLocaleString()}</span>}</div>:<form action={createOutreachMemberInvite}><input type="hidden" name="contact_id" value={contactId}/><button className="btn"><Mail size={13}/> {t('Create secure member invitation','Crear invitación segura de miembro')}</button></form>:<div className="notice error">{t('Add an email address to this Outreach person before creating a member invitation.','Agregue un correo electrónico a esta persona antes de crear una invitación de miembro.')}</div>}</div></>:<><ShieldCheck size={24}/><h2>{t('Admin invitation required','Se requiere invitación de administrador')}</h2><p className="small muted">{t('A pastor or church admin can create the secure account invitation or reconcile this guest with an existing member. Outreach leaders can continue follow-up without being able to change account identity.','Un pastor o administrador de iglesia puede crear la invitación segura o reconciliar esta visita con un miembro existente. Los líderes de alcance pueden continuar el seguimiento sin cambiar la identidad de la cuenta.')}</p></>}</section></aside></div>
  </main>
}
