...
export function OutreachHistory({contactId,interactions}:{contactId:string;interactions:any[]}){
  return <details className={styles.panel}><summary><StickyNote size={13}/> Follow-up history • {interactions.length} entr{interactions.length===1?'y':'ies'}</summary>
    ...
    {interactions.length?<div className={styles.timeline}>{interactions.slice(0,8).map((row:any)=>{const p=Array.isArray(row.profiles)?row.profiles[0]:row.profiles;return <div className={styles.event} key={row.id}><div className={styles.icon}>{icon(row.interaction_type)}</div><div className={styles.eventBody}><div className={styles.eventHead}><strong>{label(row.interaction_type)}</strong><time>{new Date(row.occurred_at).toLocaleString()}</time></div><p>{row.summary}</p><div className={styles.eventMeta}>{personName(p)}{row.bible_study_lesson!=null?` • Lesson ${row.bible_study_lesson}`:''}</div></div></div>})}</div>:<div className={styles.empty}>No logged interactions yet. The first follow-up you record will appear here.</div>}
  </details>
}
