...
const localInput=(v?:string|null)=>v?new Date(v).toISOString().slice(0,16):''

export default async function OutreachPage({searchParams}:{searchParams:Promise<{created?:string;saved?:string;error?:string}>}){
  ...
  return <main className="shell">
    <header className="topbar"><div><Link href="/" className="brand">Kingdom <span>Network</span></Link><div className="small muted">{church?.name??'Your Church'} • Outreach</div></div><Link className="ghost" href="/">← Home</Link></header>
    ...
    {query.created&&<div className="notice success">Outreach contact added.</div>}{query.saved&&<div className="notice success">Outreach record updated.</div>}{query.error&&<div className="notice error">{query.error}</div>}
