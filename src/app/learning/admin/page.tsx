...
import { BookOpen,ClipboardPlus,Eye,EyeOff,FileText,PlusCircle } from 'lucide-react'
import { createClient } from '@/lib/supabase/server'
import { addLesson,addQuestion,createAssessment,createCourse,toggleCoursePublished } from './actions'
...
export default async function LearningStudio({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const {data:coursesData}=await supabase.from('courses').select('*').eq('church_id',churchId).order('created_at',{ascending:false})
  ...
  const notices=[query.created&&'Course draft created.',query.lesson&&'Lesson added.',query.assessment&&'Assessment created.',query.question&&'Question added.',query.published&&'Course publishing status updated.'].filter(Boolean)
  ...
  return <main className="shell">
    ...
    <section className="studio-hero card"><div><div className="pill">LEARNING STUDIO</div><h1>Build discipleship pathways.</h1><p className="muted">Create lessons, attach curriculum, build assessments and publish when the course is ready.</p></div><div className="hero-stat"><strong>{courses.length}</strong><span>course{courses.length===1?'':'s'} in this church</span></div></section>
    ...
    <div className="studio-layout">
      <aside className="card studio-create"><div className="pill">NEW COURSE</div><h2>Create a draft</h2><form action={createCourse} className="studio-grid"><label className="wide"><span>Course title</span><input name="title" required placeholder="Effective Soul Winning"/></label><label><span>Category</span><input name="category" placeholder="discipleship"/></label><label><span>Estimated minutes</span><input name="estimated_minutes" type="number" min="0"/></label><label><span>Passing score</span><input name="passing_score" type="number" min="0" max="100" defaultValue="80"/></label><label><span>Credential / badge</span><input name="badge_name" placeholder="Faith Messenger"/></label><label className="wide"><span>Description</span><textarea name="description" rows={4} placeholder="What this course equips the member to do."/></label><button className="btn wide"><PlusCircle size={14}/> Create course draft</button></form><p className="small muted" style={{marginTop:10}}>New courses stay private to leadership until you publish them.</p></aside>

      <section className="studio-list">{courses.map((course:any)=>{const courseModules=modulesByCourse.get(course.id)??[];const courseAssessments=assessmentsByCourse.get(course.id)??[];return <article className="card studio-course" key={course.id}><div className="studio-course-head"><div><div className="pill">{course.category||'COURSE'}</div><h2>{course.title}</h2><p className="small muted">{course.description||'No description yet.'}</p><span className="mini-count">{courseModules.length} lessons • {courseAssessments.length} assessments • pass {course.passing_score}%</span></div><div className="studio-actions"><span className={`course-state ${course.published?'live':''}`}>{course.published?'Published':'Draft'}</span><form action={toggleCoursePublished}><input type="hidden" name="course_id" value={course.id}/><input type="hidden" name="published" value={course.published?'0':'1'}/><button className="ghost">{course.published?<><EyeOff size={13}/> Unpublish</>:<><Eye size={13}/> Publish</>}</button></form></div></div>

        <div className="studio-section"><h3><BookOpen size={14}/> Lessons</h3>{courseModules.map((module:any)=>{const moduleAssets=assetsByModule.get(module.id)??[];return <div className="lesson-editor" key={module.id}><h4>{module.position}. {module.title}</h4><p>{module.content?.summary||'No lesson summary yet.'}</p>{module.content?.body&&<p style={{whiteSpace:'pre-wrap',color:'#d9cfe0'}}>{module.content.body}</p>}{moduleAssets.map((asset:any)=><div className="question-row" key={asset.id}><FileText size={11}/> {asset.title} • {String(asset.asset_type).replaceAll('_',' ')} {asset.url&&<a href={asset.url} target="_blank" rel="noreferrer" className="record-link">Open</a>}</div>)}<AssetUploader churchId={churchId} courseId={course.id} moduleId={module.id}/></div>})}
