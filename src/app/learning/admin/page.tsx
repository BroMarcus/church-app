...
export default async function LearningStudio({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <main className="shell">
    ...
    <section className="studio-hero card"><div><div className="pill">LEARNING STUDIO</div><h1>Build discipleship pathways.</h1><p className="muted">Create bilingual, versioned courses, attach curriculum, build assessments and publish only when leadership is ready.</p></div><div className="hero-stat"><strong>{courses.length}</strong><span>course{courses.length===1?'':'s'} in this church</span></div></section>
