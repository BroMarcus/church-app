...
export default async function RewardsPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const [{data:levels},{data:xpEvents},{data:games},{data:memberBadges}]=await Promise.all([
  ...
  return <main className="shell"><header className="topbar"><div><Link href="/" className="brand">Kingdom <span>Network</span></Link><div className="small muted">Learning Rewards</div></div><div className="row"><Link className="ghost" href={`/learning/rewards?lang=${lang==='en'?'es':'en'}`}>{lang==='en'?'Español':'English'}</Link><Link className="ghost" href={`/learning?lang=${lang}`}>← Learning</Link></div></header>
    ...
    <section className="reward-section"><div className="pill">LEVEL PATH</div><h2>Your learning path</h2><div className="level-list">{levelRows.map((l:any)=><div className={`level-item ${l.level_number===current?.level_number?'current':''}`} key={l.level_number}><strong>Level {l.level_number} • {l.name}</strong><span>{l.min_xp} XP • {l.description}</span></div>)}</div></section>

    <section className="reward-section"><div className="pill">RECENT ACTIVITY</div><h2>XP history</h2><div className="card" style={{padding:16}}><div className="activity-list">{(xpEvents??[]).slice(0,15).map((e:any)=><div className="activity-row" key={e.id}><div><strong>{e.description||e.event_type.replaceAll('_',' ')}</strong><span style={{display:'block'}}>{new Date(e.created_at).toLocaleString()}</span></div><strong>+{e.points} XP</strong></div>)}{!xpEvents?.length&&<div className="small muted">Complete lessons, assessments and games to begin earning learning XP.</div>}</div></div></section>
  </main>
}
