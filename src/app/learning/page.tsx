...
export default async function LearningPage({searchParams}:{searchParams:Promise<{error?:string;lang?:string}>}){
  ...
  const card=(course:any)=>{const enrollment:any=em.get(course.id);const progress=enrollment?.progress??0;const isEs=(course.language_code??'en')==='es';const missing=missingFor(course.id);const locked=!enrollment&&missing.length>0;return <article className="card course-card" key={course.id}><div className="row" style={{gap:6,flexWrap:'wrap'}}><div className="pill">{course.category||'COURSE'}</div><div className="pill">{isEs?'ESPAÑOL':'ENGLISH'}</div><div className="pill">{audienceLabel(course.audience_level).toUpperCase()}</div>{locked&&<div className="pill"><LockKeyhole size={10}/> LOCKED</div>}</div><h2>{course.title}</h2><p>{course.description}</p><div className="course-meta"><span><Clock size={13}/> {course.estimated_minutes?Math.round(course.estimated_minutes/60)+' hrs':'Self paced'}</span><span><Award size={13}/> {course.badge_name||'Completion'}</span><span>v{course.curriculum_version??'1.0'}</span></div>{locked&&<div className="notice" style={{margin:'12px 0 0'}}><strong>{isEs?'Requisito pendiente':'Prerequisite needed'}:</strong> {missing.map((r:any)=>r.display_text).join(' • ')}</div>}<div className="progress-track"><div className="progress-fill" style={{width:`${progress}%`}}/></div><div className="progress-row"><span>{progress}% complete</span><span>{enrollment?.credential_earned?'Credential earned':enrollment?'In progress':locked?'Locked':'Not started'}</span></div>{enrollment?<Link className="btn" href={`/learning/${course.id}`} style={{display:'inline-block',marginTop:14}}>{progress?'Continue course':'Open course'}</Link>:locked?<button className="ghost" style={{marginTop:14}} disabled><LockKeyhole size={14}/> {isEs?'Completa el requisito primero':'Complete prerequisite first'}</button>:<form action={startCourse}><input type="hidden" name="course_id" value={course.id}/><button className="btn" style={{marginTop:14}}><BookOpen size={15}/> {isEs?'Comenzar curso':'Start course'}</button></form>}</article>}
