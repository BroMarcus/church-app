...
export default async function CoursePage({params,searchParams}:{params:Promise<{courseId:string}>;searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  const allClassesPassed=(modules?.length??0)>0&&requiredClassAssessments.length===(modules?.length??0)&&classTestsPassed===requiredClassAssessments.length
