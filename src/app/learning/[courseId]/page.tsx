...
export default async function CoursePage({params,searchParams}:{params:Promise<{courseId:string}>;searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  const modulePositions=new Map((modules??[]).map((m:any)=>[m.id,Number(m.position)]))
  const passedRequiredIds=new Set(requiredClassAssessments.filter((a:any)=>a.attempts.some((x:any)=>x.passed)).map((a:any)=>a.id))
  const moduleComplete=(module:any)=>{
    const required=(assessmentsByModule.get(module.id)??[]).filter((a:any)=>a.required)
    return required.length?required.every((a:any)=>passedRequiredIds.has(a.id)):completeIds.has(module.id)
  }
  const moduleUnlocked=(module:any)=>{
    if(moduleComplete(module))return true
    const position=Number(module.position)
    return requiredClassAssessments.every((a:any)=>Number(modulePositions.get(a.module_id)??Number.MAX_SAFE_INTEGER)>=position||passedRequiredIds.has(a.id))
  }
  const completedClasses=(modules??[]).filter(moduleComplete).length
  const allClassesPassed=(modules?.length??0)>0&&completedClasses===(modules?.length??0)
  const isFirstSteps=course.slug==='first-steps'
  const progress=modules?.length?Math.round((completedClasses/modules.length)*100):(enrollment?.progress??0)
  ...
  return <main className="shell">
    ...
    <section className="course-detail-hero card"><div className="row" style={{gap:6,flexWrap:'wrap'}}><div className="pill">{course.category||'COURSE'}</div><div className="pill">{isEs?'ESPAÑOL':'ENGLISH'}</div><div className="pill">VERSION {course.curriculum_version??'1.0'}</div></div><h1>{course.title}</h1><p className="muted">{course.description}</p><div className="course-meta"><span><Clock size={13}/> {course.estimated_minutes?Math.round(course.estimated_minutes/60)+' hours':'Self paced'}</span><span><Award size={13}/> {course.badge_name||'Completion credential'}</span>{course.source_revision&&<span>{course.source_revision}</span>}{requiredAssessments>0&&<span>{requiredAssessments} required test{requiredAssessments===1?'':'s'}</span>}</div><div className="progress-track"><div className="progress-fill" style={{width:`${progress}%`}}/></div><div className="progress-row"><span>{progress}% class progress</span><span>{completedClasses}/{modules?.length??0} classes complete</span></div></section>

    <section className="card" style={{padding:18,marginBottom:14}}><div className="row" style={{alignItems:'flex-start',gap:12}}><Trophy size={22}/><div><div className="pill">COMPLETION STANDARD</div><h3 style={{margin:'8px 0 6px'}}>{isFirstSteps?'Learn it. Pass the class test. Complete the class.':'Finish each section before moving forward.'}</h3><p className="muted" style={{margin:'0 0 8px'}}>{isFirstSteps?'Each First Steps class is completed only after its required class test is passed with at least 80%. After all classes are complete, the cumulative final exam unlocks.':finalExam?'Required checkpoints must be passed with at least 80%. Later sections stay locked until the earlier required checkpoint is passed, and the final unlocks after every course section is complete.':'Required assessments must be passed with at least 80% before completion is awarded.'}</p><strong>Silver 80–89% • Gold 90–99% • Platinum 100%</strong></div></div></section>

    {isFirstSteps&&<section className="card" style={{padding:18,marginBottom:14}}><div className="pill">FIRST STEPS ROADMAP</div><h2 style={{margin:'8px 0 6px'}}>{modules?.length??0} source-backed classes. One cumulative final.</h2><p className="muted" style={{marginTop:0}}>The First Steps sequence is organized from the church material provided. Complete each class in order, pass its required test, then take the cumulative final.</p><div className="row" style={{gap:10,flexWrap:'wrap',marginTop:12}}><div className="pill">{modules?.length??0} CLASSES</div><div className="pill">{lessonReadyCount}/{modules?.length??0} MATERIALS READY</div><div className="pill">{classTestsReady} REQUIRED TESTS READY</div><div className="pill">{completedClasses}/{modules?.length??0} CLASSES COMPLETE</div><div className="pill">{finalExam?`${finalExam.questions.length}-Q FINAL READY`:'FINAL PENDING'}</div></div></section>}
    ...
    <section className="course-list">{(modules??[]).map((module:any)=>{
      ...
      if(!unlocked)return <article className="card lesson-card" key={module.id}><div className="lesson-number">{module.position}</div><div className="lesson-copy"><div className="pill">LOCKED</div><h3>{module.title}</h3><p className="muted">Pass the previous required checkpoint to unlock this class.</p></div><div className="lesson-status"><LockKeyhole size={20}/></div></article>
