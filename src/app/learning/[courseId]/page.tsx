...
import { Award,CheckCircle2,Clock,Languages } from 'lucide-react'
...
export default async function CoursePage({params,searchParams}:{params:Promise<{courseId:string}>;searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  return <main className="shell">
    ...
    {credential&&<section className="course-complete card"><div className="pill">COMPLETED</div><h2>Credential earned: {course.badge_name||course.title}</h2><p>{requiredAssessments>0?'You completed every lesson and passed all required assessments.':'You completed every required lesson in this course.'} Leadership can use this verified learning record in your discipleship and ministry pathway.</p></section>}
