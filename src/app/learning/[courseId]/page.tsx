...
export default async function CoursePage({params,searchParams}:{params:Promise<{courseId:string}>;searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  return <main className="shell">
    ...
    {isFirstSteps&&<section className="card" style={{padding:18,marginBottom:14}}><div className="pill">FIRST STEPS ROADMAP</div><h2 style={{margin:'8px 0 6px'}}>17 classes. One clear discipleship path.</h2><p className="muted" style={{marginTop:0}}>The complete class sequence is in place. Full lesson material and the final exam are being built from approved church source material before students can complete the program.</p><div className="row" style={{gap:10,flexWrap:'wrap',marginTop:12}}><div className="pill">{modules?.length??0} CLASSES MAPPED</div><div className="pill">{lessonReadyCount}/{modules?.length??0} MATERIALS READY</div><div className="pill">FINAL EXAM {hasFinalExam?'READY':'PENDING'}</div></div></section>}
    ...
    <section className="course-list">{(modules??[]).map((module:any)=>{const done=completeIds.has(module.id);const summary=module.content?.summary||'Lesson content will be added by church leadership.';const materialReady=lessonMaterialReady(module);return <article className="card lesson-card" key={module.id}><div className="lesson-number">{module.position}</div><div className="lesson-copy"><div className="row" style={{gap:6,flexWrap:'wrap'}}><span className="pill">OUTLINE READY</span><span className="pill">{materialReady?'MATERIAL READY':'MATERIAL PENDING'}</span></div><h3>{module.title}</h3><p>{summary}</p>{!materialReady&&<p className="small muted">Full lesson material is still being prepared from approved church sources.</p>}</div><div className="lesson-status">{done&&<span className="complete-chip"><CheckCircle2 size={12}/> Complete</span>}<form action={setModuleComplete}><input type="hidden" name="course_id" value={courseId}/><input type="hidden" name="module_id" value={module.id}/><input type="hidden" name="complete" value={done?'0':'1'}/><button className={done?'ghost':'btn'} disabled={!enrollment||(!done&&!materialReady)}>{done?'Undo':materialReady?(isEs?'Marcar completo':'Mark complete'):'Material pending'}</button></form></div></article>})}</section>
