...
export function AssessmentCard({assessment,courseId}:{assessment:Assessment;courseId:string}){
  ...
  const isFinal=assessment.assessment_type==='final_exam'
  ...
  return <article className="card assessment-card"><div className="assessment-head"><div><div className="pill">{assessment.assessment_type.replaceAll('_',' ')}</div><h3>{assessment.title}</h3><p className="small muted">Pass with {assessment.passing_score}%{assessment.max_attempts?` • ${assessment.max_attempts} attempt${assessment.max_attempts===1?'':'s'} max`:' • unlimited attempts'}</p>{isFinal&&<p className="small"><strong>Course award:</strong> Silver 80–89% • Gold 90–99% • Platinum 100%</p>}</div><ClipboardCheck/></div>
    {attempts.length>0&&<div className="attempt-history">{attempts.map(a=>{const tier=isFinal&&a.passed?awardTier(Number(a.percentage)):null;return <span className={a.passed?'attempt-pass':'attempt-fail'} key={a.attempt_number}>Attempt {a.attempt_number}: {Number(a.percentage)}% {a.passed?(tier?`${tier} • Passed`:'Passed'):'Not passed'}</span>})}</div>}
    ...
    {result&&<div className={`assessment-result ${result.passed?'pass':'fail'}`}>{result.passed?<CheckCircle2/>:<XCircle/>}<div><strong>{result.passed?(isFinal&&awardTier(result.percentage)?`${awardTier(result.percentage)} • ${result.percentage}%`:`Passed • ${result.percentage}%`):`Keep learning • ${result.percentage}%`}</strong><span>Attempt {result.attempt_number}. {result.passed?(isFinal?'Final exam complete — your course award is based on this score.':'Great work — this assessment is complete.'):'Review the lesson and try again if another attempt is available.'}</span></div></div>}
