...
export default async function TeamsPage({searchParams}:{searchParams:Promise<{created?:string;responded?:string;error?:string;member?:string;ministry?:string}>}){
  ...
  const [{data:assignments},{data:responses},{data:ministries},{data:churchMembers}]=await Promise.all([
    supabase.from('team_assignments').select('*').eq('church_id',churchId).gte('starts_at',new Date(Date.now()-24*60*60*1000).toISOString()).order('starts_at').limit(100),
  ...
  return <main className="shell">
    <header className="topbar"><div><Link href="/" className="brand">Kingdom <span>Network</span></Link><div className="small muted">{church?.name??'Your Church'} • Teams • {timeZone.replaceAll('_',' ')}</div></div><Link className="ghost" href="/">← Home</Link></header>
    ...
    {query.created&&<div className="notice success">Assignment created.</div>}{query.responded&&<div className="notice success">Your response was saved.</div>}{handoffLoaded&&<div className="notice success">Accepted ministry applicant loaded. Add the service details below to schedule this member.</div>}{query.error&&<div className="notice error">{query.error}</div>}
