...
export default async function LoginPage({searchParams}:{searchParams:Promise<{error?:string;message?:string;invite?:string;lang?:string;mode?:string}>}){
  ...
  const query=(nextMode:string,nextLang=lang)=>`/login?lang=${nextLang}&mode=${nextMode}${params.invite?`&invite=${encodeURIComponent(params.invite)}`:''}`
  ...
  return <main className="login-wrap"><div className="login card">
    ...
    {mode==='signin'&&<>
      <form action={login}><input type="hidden" name="lang" value={lang}/><label className="field"><span>{t.email}</span><input name="email" type="email" inputMode="email" autoComplete="email" required/></label><label className="field"><span>{t.password}</span><input name="password" type="password" autoComplete="current-password" required/></label><PendingSubmit label={t.signIn} pendingLabel={t.signing}/></form>
