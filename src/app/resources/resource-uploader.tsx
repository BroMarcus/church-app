...
export function ResourceUploader({churchId,userId,canApproveOfficial}:{churchId:string;userId:string;canApproveOfficial:boolean}){
  ...
  return <form action={submit} className="card resource-upload"><div className="resource-upload-head"><div><div className="pill">ADD RESOURCE</div><h2>Archive a lesson or ministry resource</h2></div><Upload/></div><div className="resource-form-grid"><label><span>Title</span><input name="title" required placeholder="Lesson or resource title"/></label><label><span>Resource type</span><select name="resource_type" defaultValue="lesson"><option value="lesson">Lesson</option><option value="bible_study">Bible study</option><option value="teacher_guide">Teacher guide</option><option value="handout">Handout</option><option value="sermon">Sermon</option><option value="policy">Policy / constitution</option><option value="slides">Slides</option><option value="training">Training</option><option value="video">Video</option><option value="audio">Audio</option><option value="other">Other</option></select></label><label><span>Status</span><select name="archive_status" defaultValue="legacy"><option value="current">Current / approved</option><option value="legacy">Legacy / older material</option><option value="draft">Draft</option><option value="reference_only">Reference only</option><option value="retired">Retired</option></select></label><label><span>Language</span><select name="language_code" defaultValue="en"><option value="en">English</option><option value="es">Español</option><option value="bilingual">Bilingual</option></select></label><label><span>Source scope</span><select name="source_scope" defaultValue="local_church"><option value="local_church">Local church</option><option value="ministry">Ministry</option><option value="group">Group</option><option value="external">External reference</option>{canApproveOfficial&&<><option value="district">District</option><option value="organization">Organization / Assembly</option></>}</select></label><label><span>Source year</span><input name="source_year" type="number" min="1900" max="2100" placeholder="e.g. 2025"/></label><label><span>Ministry / class</span><input name="ministry_area" placeholder="First Steps, Friendship Group…"/></label><label><span>Source label</span><input name="source_label" placeholder="2021 class binder, Assembly Constitution…"/></label><label className="wide"><span>Topics / tags</span><input name="topic_tags" placeholder="prayer, baptism, holiness, outreach"/><small>Comma separated</small></label><label className="wide"><span>Scripture references</span><input name="scripture_refs" placeholder="Acts 2:38, John 3:5, Romans 6:3-4"/><small>Comma separated</small></label><label className="wide"><span>Description / notes</span><textarea name="description" rows={3} placeholder="What this resource covers and how it was used."/></label><label className="wide"><span>File</span><input name="file" type="file" required accept=".pdf,.docx,.pptx,.jpg,.jpeg,.png,.webp,.mp4,.mp3"/></label></div>{canApproveOfficial&&<label className="resource-check"><input type="checkbox" name="official_source"/><span>Mark as an official district / organization / church-approved reference. Use only for verified authoritative documents.</span></label>}<label className="resource-check"><input type="checkbox" name="approved_for_members"/><span>Members may see this resource. Leave unchecked for leadership-only legacy/reference material.</span></label><button className="btn" disabled={busy}>{busy?'Uploading…':'Save to Resource Library'}</button>{message&&<div className="small muted" style={{marginTop:8}}>{message}</div>}</form>
}
