...
export default async function ServePage({searchParams}:{searchParams:Promise<{created?:string;requirement?:string;applied?:string;reviewed?:string;error?:string}>}){
  ...
  return <main className="shell">
    ...
    {canManage&&<section className="admin-apps"><div className="section-heading"><div><div className="pill">APPLICATIONS</div><h2>Ministry applications</h2></div><span className="small muted">Leadership review</span></div>{leaderApps.map((a:any)=>{const ministry=(ministries??[]).find((m:any)=>m.id===a.ministry_id);return <article className="card application-card" key={a.id}><div><h3>{personName(pm.get(a.user_id))} → {ministry?.name||'Ministry'}</h3><div className="small muted">Submitted {new Date(a.submitted_at).toLocaleDateString()} • Qualification {a.qualification_score??0}% • {statusLabel(a.status)}</div>{a.message&&<p>{a.message}</p>}</div><form action={reviewApplication} className="review-form"><input type="hidden" name="application_id" value={a.id}/><label className="field"><span>Status</span><select name="status" defaultValue={a.status==='submitted'?'interview':a.status}><option value="qualified">Qualified</option><option value="interview">Interview</option><option value="accepted">Accepted</option><option value="declined">Declined</option></select></label><label className="field"><span>Review note</span><input name="review_note" defaultValue={a.review_note??''}/></label><button className="btn"><ShieldCheck size={14}/> Save</button></form></article>})}{!leaderApps.length&&<div className="card empty"><h3>No applications yet.</h3><p className="muted">Applications will appear here for ministry leadership.</p></div>}</section>}
  </main>
}
