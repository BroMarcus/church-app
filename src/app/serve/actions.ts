...
const serveUrl=(lang:string,extra='')=>`/serve?lang=${lang}${extra}`
...
export async function createMinistry(formData:FormData){
  const lang=langOf(formData),{supabase,userId}=await auth(lang),churchId=text(formData,'church_id'),name=text(formData,'name')
  ...
  if(!systemAccess&&!customAccess)redirect(serveUrl(lang))
  if(!name)redirect(serveUrl(lang,'&error='+encodeURIComponent(lang==='es'?'Se requiere el nombre del ministerio.':'Ministry name is required.')))
  ...
  revalidatePath('/serve');revalidatePath('/church/readiness');redirect(serveUrl(lang,'&created=1'))
}

export async function updateMinistry(formData:FormData){
  const lang=langOf(formData),{supabase}=await auth(lang),ministryId=text(formData,'ministry_id'),name=text(formData,'name')
  if(!ministryId||!name)redirect(serveUrl(lang,'&error='+encodeURIComponent(lang==='es'?'Se requiere el nombre del ministerio.':'Ministry name is required.')))
  const openings=text(formData,'openings')?Math.max(0,Number.parseInt(text(formData,'openings'),10)||0):null
  ...
  if(error)redirect(serveUrl(lang,'&error='+encodeURIComponent(error.message)))
  revalidatePath('/serve');revalidatePath('/church/readiness');redirect(serveUrl(lang,'&updated=1'))
}

export async function addRequirement(formData:FormData){
  const lang=langOf(formData),{supabase}=await auth(lang),ministryId=text(formData,'ministry_id')
  ...
  if(error)redirect(serveUrl(lang,'&error='+encodeURIComponent(error.message)))
  revalidatePath('/serve');redirect(serveUrl(lang,'&requirement=1'))
}

export async function removeRequirement(formData:FormData){
  const lang=langOf(formData),{supabase}=await auth(lang),requirementId=text(formData,'requirement_id')
  if(!requirementId)redirect(serveUrl(lang,'&error='+encodeURIComponent(lang==='es'?'No encontramos el requisito.':'Requirement not found.')))
  const {error}=await supabase.from('ministry_requirements').delete().eq('id',requirementId)
  if(error)redirect(serveUrl(lang,'&error='+encodeURIComponent(error.message)))
  revalidatePath('/serve');redirect(serveUrl(lang,'&requirement_removed=1'))
}

export async function applyToMinistry(formData:FormData){
  const lang=langOf(formData),{supabase,userId}=await auth(lang),ministryId=text(formData,'ministry_id')
  const {data:ministry}=await supabase.from('ministries').select('church_id,active').eq('id',ministryId).single()
  if(!ministry?.active)redirect(serveUrl(lang,'&error='+encodeURIComponent(lang==='es'?'Este ministerio no está aceptando solicitudes ahora.':'This ministry is not currently open for applications.')))
  ...
  if(error)redirect(serveUrl(lang,'&error='+encodeURIComponent(error.message)))
  revalidatePath('/serve');redirect(serveUrl(lang,'&applied=1'))
}
