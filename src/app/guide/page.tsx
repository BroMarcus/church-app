...
import { CalendarDays,Church,Compass,FileUp,GraduationCap,HandHeart,Languages,MessageCircle,MessageSquareWarning,Megaphone,Search,Sparkles,Users,Wrench } from 'lucide-react'
...
const memberQuick={
  en:[{title:'What matters today?',body:'See what needs your attention right now.',href:'/today',Icon:Sparkles},{title:'What is my next step?',body:'Open My Journey and see what comes next.',href:'/journey',Icon:GraduationCap},{title:'When is the next church event?',body:'See services, classes and upcoming events.',href:'/calendar',Icon:CalendarDays},{title:'How do I find my group?',body:'See your Friendship Group or browse groups.',href:'/groups',Icon:Users},{title:'I need prayer or private help',body:'Share a prayer need or contact pastoral care privately.',href:'/prayer',Icon:HandHeart},{title:'How do I contact someone?',body:'Open Messages or find someone in the church directory.',href:'/messages',Icon:MessageCircle}],
  es:[{title:'¿Qué importa hoy?',body:'Mira lo que necesita tu atención ahora.',href:'/today',Icon:Sparkles},{title:'¿Cuál es mi próximo paso?',body:'Abre Mi Camino y mira qué sigue.',href:'/journey',Icon:GraduationCap},{title:'¿Cuándo es el próximo evento?',body:'Mira servicios, clases y eventos próximos.',href:'/calendar',Icon:CalendarDays},{title:'¿Cómo encuentro mi grupo?',body:'Mira tu Grupo de Amistad o busca grupos.',href:'/groups',Icon:Users},{title:'Necesito oración o ayuda privada',body:'Comparte una petición o contacta cuidado pastoral en privado.',href:'/prayer',Icon:HandHeart},{title:'¿Cómo contacto a alguien?',body:'Abre Mensajes o busca a alguien en el directorio.',href:'/messages',Icon:MessageCircle}]
} as const
const adminQuick={
...
export default async function GuidePage({searchParams}:{searchParams:Promise<{q?:string;lang?:string}>}){
  const query=await searchParams,lang=query.lang==='es'?'es':'en',t=copy[lang],q=String(query.q??'').trim(),normalizedQuery=normalize(q),queryTokens=normalizedQuery.split(/\s+/).filter(Boolean)
  ...
  if(q){
    ...
    results=(data??[]).map((r:any)=>{
      ...
      const matchedTokens=queryTokens.filter(token=>searchable.includes(token)||title.includes(token)||description.includes(token)).length
      let score=0
      ...
      if(normalizedQuery&&searchable.includes(normalizedQuery))score+=12
      score+=matchedTokens*10+authorityScore(authority(r))+statusScore(status(r))
      return {...r,__score:score,__searchable:searchable,__matchedTokens:matchedTokens,__authority:authority(r),__status:status(r)}
