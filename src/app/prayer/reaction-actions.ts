...
export async function togglePraying(formData:FormData){
  ...
  if(!postId||postId.length>100)redirect(url({error_code:'reaction_not_found'}))
  ...
  if(!post)redirect(url({error_code:'reaction_not_found'}))
  ...
  if(existingError){console.error('prayer reaction lookup failed',{message:existingError.message});redirect(url({error_code:'reaction_failed'}))}
  if(existing?.reaction_type==='praying'){
    ...
    if(error){console.error('remove prayer reaction failed',{message:error.message});redirect(url({error_code:'reaction_failed'}))}
  }else{
    ...
    if(error){console.error('save prayer reaction failed',{message:error.message});redirect(url({error_code:'reaction_failed'}))}
  }
