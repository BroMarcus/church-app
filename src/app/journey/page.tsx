...
import { BookOpen,Check,Church,Compass,HandHeart,MessageSquareWarning,Sparkles,Users } from 'lucide-react'
...
export default async function JourneyPage({searchParams}:{searchParams:Promise<{lang?:string}>}){
  ...
  const nextByKey:any={
    newbirth:{title:t('Talk with a church leader','Habla con un líder de la iglesia'),body:t('If your baptism or Holy Ghost record is missing, a leader can help verify what belongs in your journey.','Si falta tu registro de bautismo o del Espíritu Santo, un líder puede ayudarte a verificar lo que pertenece a tu camino.'),href:`/help${lang}`,cta:t('Ask for help','Pedir ayuda')},
  ...
  return <main className="shell">
    ...
    <section className="journey-hero card"><div><div className="pill">{t('MY JOURNEY','MI CAMINO')}</div><h1>{t('Where am I—and what is next?','¿Dónde estoy y qué sigue?')}</h1><p className="muted">{t('A simple view of the spiritual and discipleship records currently connected to your account.','Una vista sencilla de los registros espirituales y de discipulado actualmente conectados a tu cuenta.')}</p></div><div className="hero-stat"><Sparkles size={23}/><span>{stages.filter(s=>s.done).length} {t('of','de')} {stages.length} {t('areas recorded','áreas registradas')}</span></div></section>
