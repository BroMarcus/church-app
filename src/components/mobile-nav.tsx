...
export function MobileNav({access,preferredLanguage='en',recoveryOnly=false}:{access:MobileNavAccess;preferredLanguage?:'en'|'es';recoveryOnly?:boolean}){
  ...
  const sections:Section[]=recoveryOnly?[{label:'Help',items:[['/start','Start Here',Sparkles],...support]}]:[{label:'Me',items:personal.filter(available)},{label:'Church',items:churchItems},...(leadership.length?[{label:'Leadership',items:leadership.filter(available)}]:[]),{label:'Help',items:support},{label:'Settings',items:settings}]
