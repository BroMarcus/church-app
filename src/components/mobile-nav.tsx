...
export function MobileNav({access,preferredLanguage='en',recoveryOnly=false}:{access:MobileNavAccess;preferredLanguage?:'en'|'es';recoveryOnly?:boolean}){
  ...
  const main:Entry[]=recoveryOnly?[['/','Home',Home]]:[['/','Home',Home],['/groups','Groups',Users],['/journey','My Journey',Sparkles],...(access.canLeadGroups?[['/groups','Leader Hub',ClipboardList] as Entry]:[['/calendar','Calendar',CalendarDays] as Entry])]
  ...
  const sections:Section[]=recoveryOnly?[{label:'Help',items:[['/start','Start Here',Sparkles],['/feedback','Help & Feedback',MessageSquareText]]}]:[{label:'Me',items:personal.filter(available)},{label:'Church',items:churchItems},...(leadership.length?[{label:'Leadership',items:leadership.filter(available)}]:[]),{label:'Help',items:support},{label:'Settings',items:settings}]
