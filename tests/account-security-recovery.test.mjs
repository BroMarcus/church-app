...
test('notification preferences keep database and query errors member-safe',async()=>{
  const page=await read('src/app/account/notifications/page.tsx')
  const actions=await read('src/app/account/notifications/actions.ts')
  assert.doesNotMatch(page,/\{query\.error\}/)
  assert.doesNotMatch(actions,/encodeURIComponent\(error\.message\)/)
  assert.match(page,/query\.error\?'generic':null/)
  assert.match(page,/Object\.prototype\.hasOwnProperty\.call\(statusCopy,query\.status\)/)
  assert.match(actions,/status=save_failed/)
  assert.match(actions,/Notification preferences save failed/)
  assert.match(actions,/Notification preferences save request failed/)
})

test('notification preferences fail closed on load errors and prevent duplicate saves',async()=>{
  const page=await read('src/app/account/notifications/page.tsx')
  const button=await read('src/app/account/notifications/notification-submit-button.tsx')
  assert.match(page,/Notification preferences lookup failed/)
  assert.match(page,/We could not load your notification preferences\./)
  assert.match(page,/No pudimos cargar tus preferencias de notificación\./)
  assert.match(page,/Nothing was changed\. Please try again\./)
  assert.match(page,/No se cambió nada\. Inténtalo otra vez\./)
  assert.match(page,/NotificationSubmitButton/)
  assert.match(page,/Saving preferences…/)
  assert.match(page,/Guardando preferencias…/)
  assert.match(button,/useFormStatus/)
  assert.match(button,/disabled=\{pending\}/)
  assert.match(button,/aria-disabled=\{pending\}/)
})
