...
8. **Resilient / duplicate-safe** — double-taps/submits, retries, refresh/back navigation, weak/interrupted network, and expired sessions must not create duplicate records or corrupt state. Protect unsaved work where practical, and make destructive actions deliberate and recoverable where possible.
9. **Accessible / language-complete** — for user-facing surfaces, verify readable text/contrast, useful labels, keyboard/focus behavior where relevant, reasonable tap targets, and no mystery icon-only actions. Where Spanish is supported, check the strategic Spanish path for untranslated text, truncation, or broken layout.
10. **Observable / safe diagnostics** — important failures should be diagnosable rather than silently swallowed, while diagnostic information remains appropriate for the type of data involved.
